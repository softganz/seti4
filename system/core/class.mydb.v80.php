...
class MyDb {
	...
	public static function value($key = NULL, $value = NULL, $escape = true) {
		...
		if ($key) {
			if (isset($value)) {
				$myDb->_values = [$key => $escape ? $myDb->mysqli->real_escape_string($value) : $value] + $myDb->_values;
			}
			...
			for ($i = 0; $i < $args_num; $i = $i + 2) {
				if (isset($args[$i]) && isset($args[$i+1])) {
					...
					if (is_null($value)) $value = "NULL";
						...
						$values = array();
						foreach (explode(',',substr($value,11)) as $v) $values[] = '"'.$myDb->mysqli->real_escape_string($v).'"';
						$value = implode(',', $values);
						//$value = implode(',', array_walk(explode(',',substr($value,11))),create_function('&$elem','$elem = mysqli_real_escape_string($myDb,$elem);'));
					} else if (is_string($value)) $value = '"'.$myDb->mysqli->real_escape_string($value).'"';
	...
	*/
	public function prepare($stmt) {
		...
		if ($myDb->status) {
			$mysqlStmt = $myDb->mysqli->stmt_init();//mysqli_stmt_init($myDb);
	...
	public static function query($stmt, $resultmode = NULL) {
		...
		if (is_string($resultmode) || is_array($resultmode) || is_object($resultmode) || strpos($stmt,'%') != false) {
			...
			if ($matches) {
				...
				$queryStmt = $prepareArgs[0] = trim($matches[1]);
		...
		if (mydb()->_watchlog) {
			if ($debug) {
				debugMsg('QUERY='.$stmt);
				debugMsg('ERROR='.$myDb->mysqli->error);//print_o(mydb(),'mydb'));
	...
	*/
	public static function select($queryStmt = NULL) {
		...
		if ($debug) debugMsg($args,'$args_before');
		if ($matches) {
			$queryStmt = $args[0] = trim($matches[1]);
			$optionPara = str_replace("'",'"',$matches[2]);
			if ($debug) debugMsg('<font color="red">STMT = '.$queryStmt.'</font>');
		}
		...
		if ($debug) debugMsg('<font color="green"><b>===> SELECT START #'.$selectCountTimes.'</b></font><br />Query : '.$queryStmt.'<br />Options : '.$optionPara.'<br />Default : '.$optionDefault.'<br />'.print_o($options,'$options'));
		if ($debug && $matches) debugMsg($matches, '$matches');
		if ($debug) debugMsg('$queryStmt = '.$queryStmt.'<br />optionPara = '.$optionPara.'<br />'.print_o($args, '$args_after'));
		...
		if ($debug) debugMsg('Prepare complete :: '.$stmt->_query).debugMsg($stmt, '$stmt');
		...
		if ($mysqlnd) {
			...
			if ($debug) debugMsg('<font color="red"><b>***** Use mysqlnd and get_result for better performance *****</b></font>');
			$stmt->execute();
		...
		$affected_rows = $myDb->mysqil->affected_rows;
		$myDb->_last_query_time = $timer->get('query',3);
		$myDb->_query_times += $myDb->_last_query_time;
