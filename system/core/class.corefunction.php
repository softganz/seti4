...
$includeFileList = [
	'system/core/lib.define.php',
	'system/core/lib.base.php',
	'system/core/lib.function.php',
	'system/core/lib.sg.php',
	'system/core/class.common.php',
	'system/core/class.module.php',
	'system/core/class.mydb.php',
	'system/core/lib.widget.php',
	'system/core/class.model.php',
	'system/core/class.view.php',
	'system/core/class.sg.php',
	'lib/class.poison.php',
	'system/model/model.user.php',
	'system/model/model.counter.php',
	// Extend Library
	'system/core/lib.corefunction.v'.cfg('core.version.major').'.php',
];
...
load_config('conf.default.php', _CORE_FOLDER.'system/core'); // load default config file
...
if (!$R->myDb->status) {
	...
	set_theme();
	die(process_index('index', message('error','OOOPS!!! Database connection error')));
}
...
class R {
	...
	public static function PageWidget($packageName, $args = []) {
		list($className, $found, $fileName, $resourceType) = load_resource_file('page.'.$packageName);
...
function import($packageName) {
	$found = false;
	list($funcName, $found, $fileName, $resourceType) = load_resource_file($packageName);
	return $found;
}
 ...
 * @param Mixed $folder
 */
function load_config($config_file = NULL, $folders = ['./system/core']) {
	$configArray = [];
...
function load_lib($file, $folder = NULL) {
	$libFile = _CORE_FOLDER.($folder ? $folder.'/' : '').$file;
	//debugMsg('LOAD LIB '.$libFile);
 ...
 * @return Mixed
 */
function load_resource_file($packageName, $debugResourceFile = false) {
	static $loadCount = 0;
	static $debugFunc = [];
	static $loadFiles = [];
	static $loadCfg = [];
	...
	// unset($debugThisResource);

	$loadCount++;

	// debugMsg('LOAD RESOURCE FILE : '.$packageName);

	if (preg_match('/^(r\.|widget\.|view\.|page\.|on\.|manifest\.|module\.)(.*)/i',$packageName,$out)) {
		$resourceType = substr($out[1],0,-1);
		$package = $out[2];
		...
		// if ($debugLoadfile) debugMsg('resourceType = <b>'.$resourceType.'</b>, packageName = <b>'.$package.'</b>, packageFolder = <b>'.$packageFolder.'</b>');
	...
	switch ($resourceType) {
		...
		case 'page' : // Page Resource
			...
			//debugMsg('$className = '.$className);
			//$paths[]='modules/'.$module.'/template';
			if (isset($request[1])) {
	...
	if (!in_array($module, $loadCfg)) {
		$cfgPaths = [$coreFolder.'/modules/'.$module, $coreFolder.'/system', '.'];
		$cfgFile = 'conf.'.$module.'.json';
		// debugMsg($cfgPaths,'$cfgPaths');
		foreach ($cfgPaths as $path) {
		 	load_config($cfgFile, $path);
		}
		$loadCfg[] = $module;
	}
	...
	if ((($funcName && function_exists($funcName)) || ($className && class_exists($className)) ) && array_key_exists($packageName, $loadFiles)) {
		...
		$debugStr .= '<div>request = '.implode(($resourceType == 'page' ? '/' : '.'),$request).'</div>';

		$debugStr .= 'Load filename : <strong>'.$fileName.'</strong>'
			. ($funcName ? ' and call function <strong>'.$funcName.'() </strong>' : '')
			. '<br />'._NL;

		$debugStr .= 'Main folder = '.$mainFolder.'<br />';
		$debugStr .= 'Path = '.implode('; ',$paths).'<br />'._NL;
...
function load_resource($packageName) {
	...
	$loadResult = list($funcName, $found, $fileName, $resourceType) = load_resource_file($packageName);
...
function process_menu($menu) {
	...
	do {
		...
		if ($debugLoadfile) {
			debugMsg('<div style="color: blue;">Load Page <b>'.$pageFile.'.php</b> in process_menu()</div>');
			// debugMsg($funcName,'$funcName');
			// debugMsg($funcArg,'$funcArg');
		}

		$loadResult = list($retClass, $found, $filename) = load_resource_file($pageFile);
...
function process_request($loadTemplate = true, $pageTemplate = NULL) {
	...
	if ($manifest[1] && $menu) { // This is a core version 4
		...
		list($exeClass, $found, $pageResultWidget) = process_menu($menu);
	} else if (function_exists('load_init_module') && $init_module_file = load_init_module(q(0))) {
		// This is a core version 3
		if ($isDebugProcess) $process_debug .= 'Load core version 3 of <b>'.$request.'</b><br />';
		list($exeClass,$pageResultWidget,$found) = process_request_v3($request);
	...
	if ($loadTemplate) echo process_index($page, $request_result);
	return $request_result;
}

function process_index($page = 'index', $text = NULL) {
	global $request_result;
	$request_result = $text;
	$result = load_template($page, NULL, false);
	return $result;
}
...
function set_theme($name = NULL, $style = 'style.css') {
	...
	*/
	...
	$template_folder[] = _CORE_FOLDER.'/system/core/';
...
/**
 * Function Deprecated : Do request process from url address and return result in string
 * @param Boolean $loadTemplate
 * @param String $pageTemplate
 * @return String
 */
function request_process($loadTemplate = false, $pageTemplate = NULL) {
	return process_request($loadTemplate, $pageTemplate);
}
