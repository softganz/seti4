		...
		if ($rs->type=='photo') {
			...
			if (file_exists($filename) and is_file($filename)) {
				...
				if (!$is_photo_inused) {
					...
					model::watch_log('project','remove photo','Photo id '.$rs->fid.' - '.$rs->file.' was removed from gallery by '.i()->name.'('.i()->uid.')');
				}
			}
		} else if ($rs->type=='doc') {
			...
			if (file_exists($filename) and is_file($filename)) {
				...
				model::watch_log('project','remove doc','File id '.$rs->fid.' - '.$rs->file.' was removed from gallery by '.i()->name.'('.i()->uid.')');
			}
		}
	}
