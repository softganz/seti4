...
if (!mydb::columns('topic','email')) {
	...
	$result[$version][]=array('Add email to topic.', R('query'), mydb()->_error, mydb()->_error_no);
}
...
if (!mydb::columns('topic_files','download')) {
	...
	$result[$version][]=array('Add download and last_download to topic_files.', R('query'), mydb()->_error, mydb()->_error_no);
}


if (!mydb::columns('tag','isdefault')) {
	mydb::query('ALTER TABLE %tag% ADD `isdefault` ENUM( "Yes" ) NULL DEFAULT NULL AFTER `vid` ');
	$result[$version][]=array('Add isdefault to tag.', R('query'), mydb()->_error, mydb()->_error_no);
}
?>