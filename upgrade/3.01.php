...
if (mydb::columns('property','value')) {
	...
	$result[$version][]=array('Change table property value from varchar to text.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (mydb::columns('topic','province')) {
	...
	$result[$version][]=array('Change field province to changwat.', R('query'), mydb()->_error, mydb()->_error_no);
}
...
if (!mydb::columns('topic','rating')) {
	...
	$result[$version][]=array('Add field rating, ratetimes.', R('query'), mydb()->_error, mydb()->_error_no);
}
...
if (!mydb::columns('topic','thread')) {
	...
	$result[$version][]=array('Add field thread to topic.', R('query'), mydb()->_error, mydb()->_error_no);

	if (mydb::table_exists('project')) {
		mydb::query('UPDATE %topic% t LEFT JOIN %project_dev% p USING(`tpid`) SET t.`thread` = p.`prevproject`');
		$result[$version][]=array('Update field topic->thread = project->prevproject.', R('query'), mydb()->_error, mydb()->_error_no);

		mydb::query('ALTER TABLE %project_dev% DROP `prevproject`;');
		$result[$version][]=array('Drop field prevproject of table project_dev.', R('query'), mydb()->_error, mydb()->_error_no);
	}
}
...
if (!mydb::columns('topic','template')) {
	...
	$result[$version][]=array('Add field template to topic.', R('query'), mydb()->_error, mydb()->_error_no);

	if (mydb::table_exists('project')) {
		mydb::query('UPDATE %topic% t LEFT JOIN %project% p USING(`tpid`) SET t.`template` = p.`template`');
		$result[$version][]=array('Update topic->template = project->template.', R('query'), mydb()->_error, mydb()->_error_no);

		mydb::query('ALTER TABLE %project% DROP `template`;');
		$result[$version][]=array('Drop field template of table project.', R('query'), mydb()->_error, mydb()->_error_no);
	}
}
...
if (!mydb::columns('topic_files','cover')) {
	...
	$result[$version][]=array('Add field cover to topic_files.', R('query'), mydb()->_error, mydb()->_error_no);
}
...
if (!mydb::columns('topic_files','tagname')) {
	if (mydb::columns('topic_files','report')) {
		...
		mydb::query($stmt);
		$result[$version][]=array('Change field report to tagname in topic_files.', R('query'), mydb()->_error, mydb()->_error_no);
	} else {
		mydb::query('ALTER TABLE %topic_files% ADD `tagname` VARCHAR(50) NULL DEFAULT NULL AFTER `type` , ADD INDEX ( `tagname` ) ');
		$result[$version][]=array('Add field tagname in topic_files.', R('query'), mydb()->_error, mydb()->_error_no);
	}

	$stmt='ALTER TABLE %topic_files% ADD INDEX (`tagname`)';
	mydb::query($stmt);
	$result[$version][]=array('Add index tagname in topic_files.', R('query'), mydb()->_error, mydb()->_error_no);
}
...
if (!mydb::columns('topic_files','orgid')) {
	...
	$result[$version][]=array('Add field orgid to topic_files.', R('query'), mydb()->_error, mydb()->_error_no);
}
...
if (!mydb::columns('topic_files','refid')) {
	...
	$result[$version][]=array('Add field refid to topic_files.', R('query'), mydb()->_error, mydb()->_error_no);
}
...
if (!mydb::columns('topic_comments','giverating')) {
	...
	$result[$version][]=array('Add field giverating to topic_comments.', R('query'), mydb()->_error, mydb()->_error_no);
}
...
if (mydb::columns('tag','vid')) {
	...
	$result[$version][]=array('Change field vid of table tag from default 0 to null.', R('query'), mydb()->_error, mydb()->_error_no);
}
...
if (!mydb::columns('tag','taggroup')) {
	...
	$result[$version][]=array('Add field taggroup into tag.', R('query'), mydb()->_error, mydb()->_error_no);
}


if (!mydb::columns('tag','process')) {
	mydb::query('ALTER TABLE %tag% ADD `process` TINYINT NULL DEFAULT NULL AFTER `taggroup` ');
	$result[$version][]=array('Tag process is optional for for tagname.', R('query'), mydb()->_error, mydb()->_error_no);
}


if (mydb::columns('users','username')) {
	mydb::query('ALTER TABLE %users% CHANGE `username` `username` VARCHAR( 30 ) NULL DEFAULT NULL ');
	$result[$version][]=array('Change size of username to 30 charactors.', R('query'), mydb()->_error, mydb()->_error_no);
}


if (mydb::columns('users','name')) {
	mydb::query('ALTER TABLE %users% CHANGE `name` `name` VARCHAR( 255 ) NULL DEFAULT "" ');
	$result[$version][]=array('Change table users field name to 255 char.', R('query'), mydb()->_error, mydb()->_error_no);
}
...
if (!mydb::columns('topic','parent')) {
	mydb::query('ALTER TABLE %topic%
								...
								ADD INDEX (`parent`);');
	$result[$version][]=array('Change table topic add field parent.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (!mydb::columns('topic','orgid')) {
	mydb::query('ALTER TABLE %topic%
								...
								ADD INDEX (`orgid`); ');
	$result[$version][]=array('Change table topic add field orgid.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (mydb::columns('topic','revid')) {
	...
	mydb::query($stmt);
	$result[$version][]=array('Change table topic change revid default to NULL.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (!mydb::columns('watchdog','keyid')) {
	...
	$result[$version][]=array('Change table watchdog add field keyid.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (!mydb::columns('watchdog','fldname')) {
	...
	$result[$version][]=array('Change table watchdog add field fldname.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (!mydb::columns('tag','catid')) {
	...
	$result[$version][]=array('Change table tag add field catid.', R('query'), mydb()->_error, mydb()->_error_no );
}


if (!mydb::columns('tag','ownid')) {
	mydb::query('ALTER TABLE %tag% ADD `ownid` INT UNSIGNED NULL DEFAULT NULL AFTER `vid` , ADD INDEX ( `ownid` ) ;');
	$result[$version][]=array('Change table tag add field ownid.', R('query'), mydb()->_error, mydb()->_error_no );
}


if (!mydb::columns('topic','title')) {
	mydb::query('ALTER TABLE %topic% ADD INDEX ( `title` ) ;');
	$result[$version][]=array('Change table topic add field title.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (!mydb::columns('tag','catparent')) {
	...
	$result[$version][]=array('Change table tag add field catparent.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (!mydb::columns('tag','listclass')) {
	...
	$result[$version][]=array('Change table tag add field listclass.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (mydb::table_exists('calendar')) {
	if (!mydb::columns('calendar','village')) {
		mydb::query('ALTER TABLE %calendar%
			...
			ADD INDEX(`tambon`),
			ADD INDEX(`village`)');
		$result[$version][]=array('Change table calendar add field village, tambon, ampur, changwat.', R('query'), mydb()->_error, mydb()->_error_no );
	}
}
...
if (mydb::table_exists('db_org') && !mydb::columns('db_org','parent')) {
	mydb::query('ALTER TABLE %db_org%
		...
		ADD INDEX(`parent`)');
	$result[$version][]=array('Change table db_org add field parent.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (mydb::table_exists('project') && !mydb::columns('project','prtype')) {
	mydb::query('ALTER TABLE %project%
		...
		ADD INDEX ( `prtype` );');
	$result[$version][]=array('Change table project add field prtype.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (mydb::table_exists('project_tr') && !mydb::columns('project_tr','refid')) {
	mydb::query('ALTER TABLE %project_tr%
		...
		ADD INDEX ( `refcode` ),
		ADD INDEX ( `tagname` );');
	$result[$version][]=array('Change table project_tr add field refid, refcode, tagname and add index.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (mydb::table_exists('db_person') && !mydb::columns('db_person','nickname')) {
	mydb::query('ALTER TABLE %person%
			...
			ADD INDEX ( `nickname` ) ;');
		$result[$version][]=array('Change table db_person add field nickname.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (mydb::table_exists('topic_user')) {
	mydb::query('ALTER TABLE %topic_user% CHANGE `membership` `membership` ENUM("Manager","Follower","Trainer","Owner","Regular member","Commentator","Viewer") NOT NULL DEFAULT "Regular member"'); 
		$result[$version][]=array('Change table topic_user change field membership.', R('query'), mydb()->_error, mydb()->_error_no );
}
...
if (mydb::table_exists('bigdata') && !mydb::columns('bigdata','fldref')) {
	mydb::query('ALTER TABLE %bigdata% ADD `fldref` VARCHAR(30) NULL DEFAULT NULL AFTER `fldtype`, ADD INDEX (`fldref`);');
		$result[$version][]=array('Change table bigdata add field fldref.', R('query'), mydb()->_error, mydb()->_error_no );
}
