...
* @return String
*/
function view_project_follow_nav($projectInfo, $options = NULL) {
	...
	if ($projectInfo->tpid) {
		if ($projectInfo->info->orgid) {
			$ui->add('<a href="'.url('project/org/'.$projectInfo->info->orgid).'"><i class="icon -material">home</i><span class="">องค์กร</span></a>');
		...
		$ui->add('<sep>');
		$ui->add('<a href="'.url('project/'.$tpid.'/result.month').'" title="ปิดงวดเดือน"><i class="icon -material">assessment</i><span class="">ปิดงวดเดือน</span></a>');
