...
function project_operate_result($self,$tpid, $period = NULL, $action = NULL, $tranId = NULL) {
	...
	$ret = '';
	//$ret .= 'Period = '.$period.' , Action = '.$action.' , TranId = '.$tranId;

	if (empty($period)) return location('project/'.$tpid.'/operate');
	...
	$url='project/'.$tpid.'/operate.result/'.$period;

	// Show form toolbar
	$ui = new Ui();
	/*
	$ui->add('<a href="'.url($url).'">รายงาน งวดที่ '.$period.'</a>');
	if ($order=='mainact') {
	...
	*/
	...
	if ($isEdit) {
		$subui=new ui();
		$subui->add('<a href="'.url('project/'.$tpid.'/operate/removeresult/'.$period).'" class="sg-action" data-confirm="ยืนยันการลบรายงาน?"><i class="icon -delete"></i><span>ลบรายงาน</span></a>');
	...
	*/
	$ret.='<h2>แบบรายงานผลการดำเนินโครงการประจำงวด '.$period.'</h2>'._NL;
