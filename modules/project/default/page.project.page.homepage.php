<?php
/**
* Project :: Page Init Command
* Created 2020-06-04
* Modify  2020-06-04
*
* @param Object $self
* @param Object $projectInfo
* @return String
*/

$debug = true;

function project_page_homepage($self, $projectInfo = NULL) {
	$tpid = $projectInfo->tpid;
	if (!$tpid) return message('error', 'PROCESS ERROR');

	$isAdmin = user_access('access administrator pages');

	if (!$isAdmin) return message('error', 'Access Denied');

	$homeCmdKey='project:HOMEPAGE:'.$tpid;

	if (post('homepage')) {
		property($homeCmdKey,post('homepage'));
		return 'SAVED';
	}

	$ret = '<header class="header">'._HEADER_BACK.'<h3>Project Home Page Command</h3></header>';

	$form = new Form(NULL,url('project/'.$tpid.'/page.homepage'), NULL, 'sg-form');
	$form->addData('rel', 'notify');
	$form->addData('done', 'reload');

	$homeCmd=property($homeCmdKey);

	$form->addField(
		'homepage',
		array(
			'type'=>'textarea',
			'label'=>'Home Page Command',
			'class'=>'-fill',
			'rows'=>20,
			'value'=>htmlspecialchars($homeCmd),
		)
	);

	$form->addField(
		'save',
		array(
			'type'=>'button',
			'value'=>'<i class="icon -save -white"></i><span>{tr:Save}</span>',
		)
	);

	$ret.=$form->build();
	return $ret;
}
?>