...
function view_project_action_render($projectInfo, $actionInfo) {
	...
	$drafAction = $actionInfo->flag === _PROJECT_DRAFTREPORT;
	$lockFinancial = $actionInfo->flag === _PROJECT_LOCKREPORT;
	...
	$inlineAttr['class'] .= ' -'.($drafAction ? 'draft' : $actionInfo->part);
	...
	$profileStr .= ($actionInfo->username ? '<a class="sg-action" href="'.url('profile/'.$actionInfo->uid).'" data-rel="box" data-width="640">' : '').$actionInfo->ownerName.($actionInfo->username?'</a>' : '');
	$profileStr .= '</span>';
	...
	$profileStr .='<div style="clear:both;">'
					...
					.($drafAction ? '<br /><font color="red">ร่างรายงาน</font> ' : '')
					.($drafAction && $isItemEdit ? ' - <a href="'.url('paper/'.$actionInfo->tpid.'/'.$part, array('act' => 'addreport', 'trid' => $actionId)).'" title="แก้ไขร่างบันทึกกิจกรรม">แก้ไขร่างบันทึกกิจกรรม</a>' : '')
					.'</div>';
	...
	if ($isEdit) {
		if ($isItemEdit) {
			$ui->add('<a class="sg-action" href="'.url('project/'.$actionInfo->tpid.'/info/action.post/'.$actionInfo->actionId).'" data-rel="box" data-width="640" title="แก้ไข'.($drafAction ? 'ร่าง' : '').'บันทึกกิจกรรม">'
				.'<i class="icon -material">edit</i>'
				.'<span>แก้ไข'.($drafAction ? 'ร่าง' : '').'บันทึกกิจกรรม</span>'
				.'</a>');
	...
	if ($isItemEdit) {
		$actStr .= '<form class="sg-upload -no-print" method="post" enctype="multipart/form-data" '
			...
			.'<span>ส่งภาพถ่ายกิจกรรมหรือไฟล์รายงาน</span>'
			.'<input type="file" name="photo[]" multiple="true" class="inline-upload -actionphoto" />'
			.'</span>'
			.'</form>'._NL
			. '<p class="-sg-text-center -noprint"><em>"ภาพถ่ายกิจกรรมหรือไฟล์รายงาน" จะแสดงให้บุคคลทั่วไปเห็น ห้ามส่งภาพที่มีรายละเอียดส่วนบุคคล เช่น เลขที่บัตรประชาชน เบอร์โทรศัพท์ โดยเด็ดขาด</em></p>';
	}
	...
	if ($showBudget && $actionInfo->part == 'owner') {
		$moneyTable = new Table([
			'class' => 'project-action-money-table -center',
			'caption' => 'รายงานการใช้เงิน',
			'thead' => ['ค่าตอบแทน', 'ค่าจ้าง', 'ค่าใช้สอย', 'ค่าวัสดุ', 'ค่าสาธารณูปโภค', 'อื่น ๆ', 'รวมรายจ่าย', 'สถานะ'],
			'children' => [
				[
					...
					$isAdmin ? '<a href="'.url('project/edit/lockmoney/'.$actionId).'" class="project-lockmoney"><i class="icon -material'.($lockFinancial ? '' : ' -gray').'">'.($lockFinancial ? 'lock' : 'lock_open').'</i></a>' : '<i class="icon -material'.($lockFinancial ? '' : ' -gray').'">'.($lockFinancial ? 'lock' : 'lock_open').'</i>',
				],
			], // children
		]);
