...
function r_project_action_get2($conditions = NULL, $options = '{}') {
	...
	if (is_string($conditions) && preg_match('/^{/',$conditions)) {
		...
		$conditions = (Object) ['projectId' => $conditions];
	}
	...
	if (empty($conditions->actionId)) unset($conditions->actionId);
	...
	//if (empty($projectId)) return $result;
	...
	*/
	...
	// 			FROM %topic_files% ph
	// 			WHERE (ph.`type` = "photo" AND (ph.`tagname` = "project,action" AND ph.`refid` = ac.`trid`)) ) `photos`
	// --	, ( SELECT COUNT(*)
	// --			FROM %topic_files%
	...
	$stmt = 'SELECT
		-- GET 2
		a.*
		, GROUP_CONCAT(f.`fid`,"|",f.`file`) `photos`
		FROM (SELECT
		  ac.`tpid` `projectId`
		, ac.`tpid`
		, ac.`trid` `actionId`
		...
		ORDER BY $ORDER$
			...
			LEFT JOIN %topic_files% f ON f.`type` = "photo" AND f.`tagname` = "project,action" AND f.`refid` = a.`actionId`
		GROUP BY `actionId`
