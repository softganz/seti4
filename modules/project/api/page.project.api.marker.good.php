...
* @return String
*/
function project_api_marker_good($self) {
	$getSearch = post('q');
	$group = post('g');
	$getKeyword = post('kw');
	...
	*/
	...
	mydb::where('p.`prtype` = "โครงการ" AND p.`location` IS NOT NULL');
	...
	if ($getKeyword) mydb::where('tr.`text1` LIKE :keyword',':keyword','%'.$getKeyword.'%');
