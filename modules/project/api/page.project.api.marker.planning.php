...
* @return String
*/
function project_api_marker_planning($self) {
	$getSearch = post('q');
	...
	*/
	...
	mydb::where('p.`prtype` = "แผนงาน" AND p.`location` IS NOT NULL');
	...
	$stmt = 'SELECT
					...
					FROM %project% p
						...
						LEFT JOIN %project_tr% pl ON pl.`tpid` = p.`tpid` AND pl.`formid` = "info" AND pl.`part` = "title"
