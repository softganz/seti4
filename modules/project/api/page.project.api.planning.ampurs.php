...
* @return JSON
*
* @usage project/api/planning/ampurs
*/

$debug = true;

function project_api_planning_ampurs($self, $param = NULL) {
	$getGroupBy = SG\getFirst(post('group'),'year');

	$filterPlan = SG\getFirst($param->filterPlan, post('plan'), implode(',',post('for_plan')));
	$filterProblem = SG\getFirst($param->filterProblem, post('problem'), implode(',',post('for_problem')));
	...
	$filterSector = SG\getFirst($param->filterSector, post('sector'),implode(',',post('for_sector')));
	...
	// Filter
	mydb::where('(p.`prtype` = "แผนงาน" AND tr.`formid` LIKE "info" AND tr.`part` LIKE :part AND tr.`refid` IS NOT NULL AND tr.`num1` IS NOT NULL AND tr.`num2` IS NOT NULL)', ':part', "problem");
	...
	*/
	...
	$stmt = 'SELECT
		...
		%WHERE%
		GROUP BY $GROUPBY$
		ORDER BY `year` ASC, `problemId` ASC
		';
