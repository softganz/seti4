 ...
 * @param String $report
 */
function project_form_s1($self,$topic,$para=NULL,$report=NULL) {
	$tpid=$topic->tpid;
	$formid='ส.1';
	$period=$para->period;
	$action=post('act');
	...
	// Show form toolbar
	$ui=new ui();
	$ui->add('<a href="'.url($url).'">รายงานความก้าวหน้าโครงการ (ส.1) งวดที่ '.$period.'</a>');
	if ($order=='mainact') {
		$ui->add('<a href="'.url($url,array('order'=>'date')).'">เรียงกิจกรรมตามวันที่</a>');
	} else {
		$ui->add('<a href="'.url($url).'">เรียงกิจกรรมตามกิจกรรมหลัก</a>');
	}
	$ui->add('<a href="javascript:void(0)">สถานะรายงาน : '.($locked?'Locked':'Unlock').'</a>');
	...
	$ret.='<p>1. ชื่อโครงการ <strong>'.$topic->title.'</strong></p>'._NL;
	$ret.='<p>2. ชุมชน <strong>'.$topic->project->area.'</strong></p>'._NL;
	$ret.='<p>3. รหัสโครงการ <strong>'.$topic->project->prid.'</strong> เลขที่ข้อตกลง <strong>'.$topic->project->agrno.'</strong></p>'._NL;
	$ret.='<p>4. ระยะเวลาดำเนินงาน ตั้งแต่ <strong>'.sg_date($topic->project->date_from,'ว ดดด ปปปป').' ถึง '.sg_date($topic->project->date_end,'ว ดดด ปปปป').'</strong></p>'._NL;
	$ret.='<p>5. รายงานงวดที่ : <strong>'.$period.'</strong> จากเดือน <strong>'.sg_date($periodInfo->report_from_date,'ดดด ปปปป').'</strong> ถึงเดือน <strong>'.sg_date($periodInfo->report_to_date,'ดดด ปปปป').'</strong></p>'._NL;
	...
	$ret.='<div style="width:40%;float:right;margin:60px 5% 30px;text-align:center;"><p>(................................)<br />ทีมสนับสนุนวิชาการ ส.น. 6<br />......./............/.......</p></div>'._NL;
...
function __project_form_s1_addleader($tpid,$period) {
	$backUrl='#close';
	//if ($action!='info') $backUrl=url('project/mainact/'.$tpid.'/info/'.$actid);
	...
	$form = new Form('person', url('paper/'.$tpid.'/owner/s1/period/'.$period), 'org-add-person', 'sg-form');
...
function __project_form_s1_listleader($tpid,$period,$isEdit) {
	...
	$tables = new Table();
	$tables->thead=array('ชื่อ-สกุล','ที่อยู่ติดต่อได้สะดวก','บทบาทแกนนำ','center'=>$isEdit?'<a class="sg-action" href="'.url('project/form/'.$tpid.'/s1/period/'.$period,array('act'=>'addleader')).'" data-rel="box" data-width="640" title="เพิ่มรายชื่อแกนนำ"><i class="icon -add"></i></a>':'');
	...
	if ($isEdit) {
		$tables->rows[]=array('<td colspan="4" class="noprint" style="text-align:center;"><a class="sg-action btn" href="'.url('project/form/'.$tpid.'/s1/period/'.$period,array('act'=>'addleader')).'" data-rel="box" data-width="640" title="เพิ่มรายชื่อแกนนำ"><i class="icon -person-add"></i>เพิ่มรายชื่อแกนนำ</a></td>');
	}
...
* @return String
*/
function __project_form_s1_addinno($tpid,$trid = NULL) {
	$period=post('period');
	...
	$form = new Form('inno', url('paper/'.$tpid.'/owner/s1'), 'org-add-person', 'sg-form');
	$form->config->attr='data-rel="box" data-done="close" data-callback="refreshContent" data-refresh-url="'.url('paper/'.$tpid.'/owner/s1/period/'.$period).'"';
...
function __project_form_s1_listinno($tpid,$topic,$period,$isEdit) {
	...
	$tables->thead=array(
									...
									$isEdit?'<a class="sg-action -no-print" href="'.url('project/form/'.$tpid.'/s1',array('act'=>'addinno','period'=>$period)).'" data-rel="box" title="เพิ่มรายชื่อนวัตกรรม"><i class="icon -add"></i></a>':''
									);
