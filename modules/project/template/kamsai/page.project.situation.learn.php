...
* @return String
*/
function project_situation_learn($self) {
	project_model::set_toolbar($self,'ผลสัมฤทธิ์ทางการเรียนของนักเรียน');
	...
	$zoneList=cfg('zones');
	$formText.='<form method="get" action="'.url('project/situation/weight').'">';
	if ($zoneList) {
	...
	$formText.='<select name="province" class="form-select" onchange="notify(\'กำลังโหลด\');$(this).closest(\'form\').submit();return false;"><option value="">ทุกจังหวัด</option>';
	...
	$tables->caption='ภาวะโภชนาการ (น้ำหนักตามเกณฑ์ส่วนสูง)';
	...
	$tables=new table('item -weightform');
	$tables->caption='ภาวะโภชนาการ (ส่วนสูงตามเกณฑ์อายุ)';
	$tables->colgroup=array('','amt student'=>'','amt bad'=>'','amt badpercent'=>'','amt fair'=>'','amt fairpercent'=>'','amt good'=>'','amt goodpercent'=>'');
