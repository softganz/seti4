...
* @return String
*/
function project_report_map_goodcenter($self) {
	$goodGroup=post('g');
	...
	foreach ($dbs->items as $rs) {
		if ($rs->lat && $rs->lng) {
			...
			$gis['markers'][]=array(
														'latitude'=>$rs->lat,
														'longitude'=>$rs->lng,
														'provname'=>$rs->provname,
														'point'=>$rs->totalPoint,
														'title'=>$rs->title,
														'icon'=>$icon,
														'content'=>'<h4>'.$rs->title.'</h4><p>คุณค่าที่เกิดขึ้น : '.$parts[$rs->part].'<br />สถานภาพ : '.$rs->project_status.'<br /><a href="'.url('paper/'.$rs->tpid).'">รายละเอียดโครงการ</a> | <a href="'.url('paper/'.$rs->tpid.'/situation/kamsaiindicator').'">ประเมินศูนย์เรียนรู้</a></p>'
														);
			*/
			$gis['markers'][]=array(
														$rs->provname,
														intval($rs->totalPoint),
														);
		...
		$tables->rows[]=array(++$no,
												...
												$rs->provname,
												'<a href="'.url('paper/'.$rs->tpid.'/situation/kamsaiindicator').'">'.$rs->title.'</a>',$rs->project_status,
												'config'=>array('class'=>'project-status-'.$rs->project_statuscode));
	}

	$ret.='<ul class="ui-menu">'._NL;
	foreach ($guideList as $k=>$v) {
		$ret.='<li><a href="'.url('project/report/map/goodcenter','g='.$k).'"><span class="" style="width:20px;height:20px;display:inline-block;background:#FD675B;border-radius:20px;border:2px #fff solid; text-align:center; line-height:20px;">'.substr($k,-1).'</span> '.$v['title'].'</a>';
		if ($v['items']) {
			$ret.='<ul>';
			foreach ($v['items'] as $k2 => $v2) {
				$ret.='<li><a href="'.url('project/report/map/goodcenter',array('g'=>$k,'s'=>$k2)).'">'.$v2.'</a></li>';
			}
			$ret.='</ul>';
		}
		$ret.='</li>'._NL;
