<?php
	 ...
	 *
	 */
	function project_report_southplan($self) {
		$self->theme->title='รายงานโครงการแยกตามความสอดคล้องกับยุทธศาสตร์และแผนปฏิบัติการพัฒนาจังหวัดชายแดนภาคใต้';
		...
		if (!$relId) {
			$stmt='SELECT t.`title`, tr.`part`, tg.`tid` relId, tg.`name` `relName`, tg.`taggroup`,
				...
				WHERE `taggroup`="project:rel-southplan"
				GROUP BY `taggroup`,tr.`parent`';
			$dbs=mydb::select($stmt);

			$tables = new Table();
			$tables->thead=array('แผน', 'amt projects'=>'โครงการ','money budgets'=>'งบประมาณ(บาท)');
			foreach ($dbs->items as $rs) {
				$departmentName=SG\getFirst($rs->relName,'ไม่ระบุ');
				$tables->rows[]=array('<a href="'.url('project/report/southplan',array('rel'=>$rs->relId)).'" class="sg-action" data-rel="#info">'.$departmentName.'</a>',$rs->amt,number_format($rs->totalBudget,2));
		...
		*/
		...
		mydb::where('`taggroup` = "project:rel-southplan"');
