<?php
	 ...
	 *
	 */
	function project_report_kpi($self) {
		$self->theme->title='ความสอดคล้องกับตัวชี้วัดแผนงานการแก้ปัญหาจังหวัดชายแดนภาคใต้';
		...
		if (!$relId) {
			$stmt='SELECT t.`title`, tr.`part`, tg.`tid` relId, tg.`name` `relName`, tg.`taggroup`,
				...
				WHERE `taggroup`="project:rel-kpi"
			...
			foreach ($dbs->items as $rs) {
				...
				$tables->rows[]=array('<a href="'.url('project/report/kpi',array('rel'=>$rs->relId)).'" class="sg-action" data-rel="#info">'.$departmentName.'</a>',$rs->amt,number_format($rs->totalBudget,2));
		...
		*/
		...
		mydb::where('`taggroup` = "project:rel-kpi"');
