...
function r_project_qt_people_save($data) {
	$result = NULL;
	$result-> _error = false;
	...
	$data->qtform = _PROJECT_QTFORM_PEOPLE;
	...
	if ($tranData->{"EDUCATION-OTHER"}) {
		$tranData->EDUCATION=$tranData->{"EDUCATION-OTHER"};
	}
	unset($tranData->{"EDUCATION-OTHER"});
	...
	$oldData = R::Model('project.qt.people.get', $data->tpid, $data->qtref);
