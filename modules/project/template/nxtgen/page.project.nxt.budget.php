...
class ProjectNxtBudget extends Page {
	var $courseId;
	var $year;
	var $orgId;
	var $title;
					...
					'children' => [
						...
						'year' => [
							...
							'options' => ['' => '=ทุกปี=='] + mydb::select(
								...
								FROM %project% p
									LEFT JOIN %topic% t ON t.`tpid` = p.`tpid`
								WHERE t.`parent` IN (:parent) AND p.`project_status` IN ( 1 )
								GROUP BY `pryear`
				...
				'children' => [
					new Table([
						'thead' => [
							...
							'period -amt' => 'งวด',
							'period-budget -money' => 'รวมเงินงวด(บาท)'
						],
						'children' => array_map(
							function($item) {
								return [
										'<a href="'.url('project/'.$item->projectId.'/info.nxt.period').'">'.$item->title.'</a>'
										...
										$item->periodCount,
										number_format($item->periodBudget,2),
									];
							},
