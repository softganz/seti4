...
* @return String
*/
function view_project_nav_assessor($rs,$options) {
	$ret='';
	$ui=new Ui(NULL,'ui-nav -info');
	$ui->add('<a href="'.url('project/my').'"><i class="icon -home"></i><span class="-hidden">โครงการ</span></a>');
	...
	if ($isRegister) {
		...
		$ui->add('<a class="btn -primary" href="'.url('project/assessor/register').'" style="border-radius: 8px; box-shadow: none; border: none;"><i class="icon -addbig -circle"></i><span>ลงทะเบียนนักติดตามประเมินผล</span></a>');
	}
	$ret.=$ui->build();
