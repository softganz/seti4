...
function view_project_nav_proposal($proposalInfo, $options = NULL) {
	...
	if ($tpid) {
		if ($proposalInfo->info->thread) {
			...
			$ui->add('<a href="'.url('project/'.$proposalInfo->info->parent.'/page').'"><i class="icon -material">arrow_back</i><span>BACK</span></a>');
		}
	...
	if (i()->ok) {
		...
		$dropboxMember->add('<a class="sg-action" href="'.url('project/'.SG\getFirst($tpid,$proposalInfo->set).'/info.u/'.i()->uid).'" data-rel="box" data-width="640" title="ข้อเสนอของฉัน"><i class="icon -material -sg-16">assessment</i><span>ข้อเสนอของฉัน</span></a>');
