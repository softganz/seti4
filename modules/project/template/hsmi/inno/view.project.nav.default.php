...
function view_project_nav_default($projectInfo, $options = NULL) {
	...
	if ($projectInfo->submodule == 'set') {
		...
		$homeUrl = $projectInfo->info->parent == 393 ? 'par' : 'inno';
		$ui->add('<a href="'.url($homeUrl).'"><i class="icon -material">home</i><span class="">หน้าแรก</span></a>');
	...
	if (i()->ok) {
		...
		$dropboxMember->add('<a class="sg-action" href="'.url('project/'.$setId.'/info.u/'.i()->uid).'" data-rel="box" data-width="640" title="{tr:โครงการ}ของฉัน"><i class="icon -material -sg-16">assessment</i><span>{tr:โครงการ}ของฉัน</span></a>');
