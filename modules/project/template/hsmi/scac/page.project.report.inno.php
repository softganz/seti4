...
function project_report_inno($self) {
	project_model::set_toolbar($self,'การเกิดขึ้นของนวัตกรรม');
	...
	$form->config->variable='report';
	$form->config->method='get';
	$form->config->action=url(q());
	$form->config->class='form -inlineitem';
	...
	$ret.='<div class="toolbar -sub">'.theme('form','project-report',$form).'</div>';
...
//		$ret.=print_o($yearDbs);
	...
	$ret.='<div id="project-report-detail">'.theme('table',$tables).'</div>';
...
var maxProject='.$max.'
var options = {
								title: "การเกิดขึ้นของนวัตกรรม'.($inno?' '.$parts[$inno]:'').($province?' จังหวัด'.$form->province->options[$province]:'').'",
