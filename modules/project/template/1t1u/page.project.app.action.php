...
class ProjectAppAction extends Page {
	function build() {
		...
		if ($isEmployee) {
			if ($actionSelectCard->count()) {
				$nav[] = '<a class="sg-action" href="#project-app-action-select" data-rel="box" data-width="480"><i class="icon -material">add_task</i><span>ส่งรายงาน</span></a>';
			} else {
				$nav[] = '<a class="sg-action" href="'.url('project/app/month').'" data-webview="ส่งรายงานประจำเดือน"><i class="icon -material">add_task</i><span>ส่งรายงาน</span></a>';
			}
		}
		if ($followSelectCard->count()) {
			$nav[] = '<a class="sg-action" href="#project-app-follow-select" data-rel="box" data-width="480"><i class="icon -material">dashboard</i><span>โครงการ</span></a>';
		} else {
			$nav[] = '<a class="sg-action" href="'.url('project/app/follow/my').'" data-webview="โครงการ"><i class="icon -material">dashboard</i><span>โครงการ</span></a>';
		}
		$nav[] = '<a class="sg-action" href="'.url('project/app/calendar').'" data-webview="ปฎิทิน"><i class="icon -material">event</i><span>ปฎิทิน</span></a>';
			...
			'body' => new Widget([
				'children' => [
					...
					new Container([
						...
						'class' => 'sg-load',
						'attribute' => ['data-url' => url('project/app/activity', array('u' => i()->uid))],
						'child' => '<div class="loader -rotate" style="width: 64px; height: 64px; margin: 32px auto; display: block;"></div>',
					]), // Container
