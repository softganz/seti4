...
* @return String
*
* @usage project/{id}/info.child.address.edit/{childId}
*/

$debug = true;

function project_info_child_address_edit($self, $projectInfo, $childId) {
	// Data Model
	...
	return R::Page('project.info.address.edit', NULL, $projectInfo);
	...
	// View Model
	$toolbar = new Toolbar($self, 'ที่อยู่ผู้รับจ้าง - '.$projectInfo->title);
	...
	$childDbs = mydb::select(
		...
		, p.`ownertype`
		, CONCAT(pn.`prename`, pn.`name`, " ", pn.`lname`) `fullName`
		, pn.`cid`, pn.`phone`
		, c.`fldref` `bankChackStatus`
		, tp.`title` `parentTitle`
		, pn.`house`, pn.`areacode`
	...
	$tables->thead = array(
		...
		'bankaccount -nowrap' => 'ชื่อ - นามสกุล',
		'address' => 'ที่อยู่',
		'action -noprint' => ''
	);
	...
	foreach ($childDbs->items as $rs) {
		...
		$row = array(
			...
			$rs->fullName,
			$rs->house,
			'<nav class="nav -icons"><ul>'
			//. '<li><a class="btn -link">'.$checkIcon.'</a></li>'
			. '<li><a class="sg-action btn -link" href="'.url('project/'.$projectId.'/info.child.address.edit/'.$rs->projectId).'" data-rel="box" data-width="480"><i class="icon -material">edit</i></a></li></ul></nav>',
		);
