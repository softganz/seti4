...
function project_develop_view($self, $tpid, $action = NULL) {
	//R::SetPage('__view');
	...
	$isEditable = $devInfo->RIGHT & _IS_EDITABLE && (post('mode') != 'view');
	...
	/*
	preg_match_all('|<span.*?data-fld=\"([a-zA-Z0-9\-].*?)\".*?>(.*?)</span>|s',$body,$matches);
	foreach ($matches[1] as $key => $value) {
		$ret.=$value.'='.$matches[2][$key].'<br />'._NL;
	}
	...
	*/
	...
	if (!$devInfo->info->orgid && ((i()->ok && $devInfo->info->uid == i()->uid)) ) {
		$ui->add('<a class="btn -primary" href="'.url('project/develop/tofund/'.$tpid).'" title="ส่งโครงการ"><i class="icon -forward -white"></i><span class="">ส่งโครงการ</span></a>&nbsp;');
	}

	if ($devInfo->info->toorg && empty($devInfo->info->orgid)) {
		$orgInfo = R::Model('org.get', $devInfo->info->toorg);
		if ($orgInfo->RIGHT & _IS_ADMIN) {
			$ui->add('<a class="sg-action btn" href="'.url('project/develop/addtoorg/'.$tpid).'" data-rel="box" data-width="640"><i class="icon -addbig"></i><span>รับโครงการไว้พิจารณา</span></a>');
		}
	}


	list($oldProject,$oldDevelop)=explode(',', $info->homepage);

	// FIXME: Bug for PA Develop project nav bar <!--?php for nav
	$ret .= '<nav></nav>';
	...
	if ($isViewOnly) {
		...
		$ret .= '<div class="btn-floating -right-bottom">'
			. '<a class="sg-action btn -floating -circle48" href="'.url('project/develop/'.$tpid,array('debug'=>post('debug'))).'" data-rel="#main"><i class="icon -material -white">done_all</i></a>'
			. '</div>';
	} else if ($isEditable) {
		$ret .= '<div class="btn-floating -right-bottom">'
			. '<a class="sg-action btn -floating -circle48" href="'.url('project/develop/'.$tpid.'/view/edit',array('debug'=>post('debug'))).'" data-rel="#main"><i class="icon -edit -white"></i></a>'
			. (empty($devInfo->info->followId) ? '<a class="sg-action btn -floating -circle48" href="'.url('project/develop/'.$tpid.'/delete').'" title="ลบโครงการกำลังพัฒนา" data-rel="none" data-done="reload:'.url('project/fund/'.$devInfo->orgid.'/proposal').'" data-title="ลบพัฒนาโครงการ" data-confirm="ต้องการลบพัฒนาโครงการ กรุณายืนยัน?"><i class="icon -material">delete</i></a>' : '')
			. '</div>';
	}
