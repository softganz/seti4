...
* @return String
*
* @call project/fund/$orgId/board.all[/$series]
*/

$debug = true;

function project_fund_board_all($self, $fundInfo, $series = NULL) {
	...
	$stmt = 'SELECT b.*, bp.`name` `boardName`
		, p.`name` `positionName`, bs.`name` `boardStatus`, p.`weight`
		FROM %org_board% b
			...
			LEFT JOIN %tag% bs ON bs.`taggroup` = "boardstatus" AND bs.`catid` = b.`status`
		%WHERE%
	...
	foreach ($dbs->items as $rs) {
		...
		if ($isEditable) {
			$ui = new Ui('span');
			$ui->add('<a class="sg-action" href="'.url('project/fund/'.$fundInfo->orgid.'/board.edit/'.$rs->brdid).'" data-rel="box" data-width="470px"><i class="icon -edit"></i></a>');
			$ui->add('<a class="sg-action" href="'.url('project/fund/'.$fundInfo->orgid.'/info/board.delete/'.$rs->brdid).'" data-rel="notify" data-title="ลบกรรมการ" data-confirm="ต้องการลบกรรมการ กรุณายืนยัน?" data-done="remove:parent tr"><i class="icon -material -gray">cancel</i></a>');
