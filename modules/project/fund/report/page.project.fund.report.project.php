...
* @return String
*
* @usage project/fund/report/project
*/

$debug = true;

function project_fund_report_project($self) {
	$year=post('yr');
	$area=post('area');
	$prov=post('prov');
	$ampur=post('ampur');
	...
	$repTitle='รายงานสรุปจำนวนแผนงาน/โครงการ/กิจกรรมของกองทุนฯ';

	R::view('project.toolbar',$self,'รายงาน - '.$repTitle,'fund');

	$ui=new Ui();
	$ui->add('<a class="btn" href="'.url('project/report').'">รายงาน</a>');
	$ui->add('<a class="btn" href="'.url('project/fund/report/project').'">'.$repTitle.'</a>');
	$ret.='<nav class="nav -page">'.$ui->build().'</nav>';

	$form='<form id="condition" action="'.url('project/fund/report/project').'" method="get">';
	...
	$stmt="SELECT
				...
				, COUNT(IF(p.`supporttype`=1,1,NULL)) `totalType1`
				, COUNT(IF(p.`supporttype`=2,1,NULL)) `totalType2`
				, COUNT(IF(p.`supporttype`=3,1,NULL)) `totalType3`
				, COUNT(IF(p.`supporttype`=4,1,NULL)) `totalType4`
				, COUNT(IF(p.`supporttype`=5,1,NULL)) `totalType5`
				, COUNT(IF(p.`tpid` IS NOT NULL AND (p.`supporttype`='' OR p.`supporttype` IS NULL),1,NULL)) `totalNA`
				FROM %project_area% a
	...
	foreach ($dbs->items as $rs) {
		...
		$chartTable->rows[]=array(
			...
			'number:ประเภทที่ 1'=>$rs->totalType1,
			'string:ประเภทที่ 1:role'=>$rs->totalType1>=$numberToShow?number_format($rs->totalType1):'',
			'number:ประเภทที่ 2'=>$rs->totalType2,
			'string:ประเภทที่ 2:role'=>$rs->totalType2>=$numberToShow?number_format($rs->totalType2):'',
			'number:ประเภทที่ 3'=>$rs->totalType3,
			'string:ประเภทที่ 3:role'=>$rs->totalType3>=$numberToShow?number_format($rs->totalType3):'',
			'number:ประเภทที่ 4'=>$rs->totalType4,
			'string:ประเภทที่ 4:role'=>$rs->totalType4>=$numberToShow?number_format($rs->totalType4):'',
			'number:ประเภทที่ 5'=>$rs->totalType5,
			'string:ประเภทที่ 5:role'=>$rs->totalType5>=$numberToShow?number_format($rs->totalType5):'',
	...
	$ret.='<div id="fund-type" class="sg-chart -type" data-chart-type="pie"><h3>แผนภูมิแสดงจำนวนโครงการของกองทุนฯ</h3>'.$chartPie->build().'</div>'._NL;
	...
	$ret.='<div id="fund-join" class="sg-chart -join" data-chart-type="col" data-options=\''.json_encode($options).'\'><h3>แผนภูมิแสดงจำนวนแผนงาน/โครงการ/กิจกรรมของกองทุนฯ</h3>'.$chartTable->build().'</div>'._NL;
