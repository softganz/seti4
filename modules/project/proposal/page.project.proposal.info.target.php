...
* @return String
*/
function project_proposal_info_target($self, $proposalInfo, $action = NULL, $tranId = NULL) {
	...
	$ret .= '<div id="project-develop-target" data-url="'.url('project/proposal/'.$tpid.'/info.target/'.$action).'">';
	...
	foreach ($targetList as $targetGroup) {
		...
		foreach ($targetGroup as $key => $targetItem) {
			...
			if ($isEdit && $targetItem->tgtid) {
				...
				$menu .= '<a class="sg-action" href="'.url('project/proposal/'.$tpid.'/info/target.delete',array('id'=>$targetItem->tgtid)).'" data-title="ลบกลุ่มเป้าหมาย" data-confirm="ต้องการลบกลุ่มเป้าหมายรายการนี้ กรุณายืนยัน?" data-rel="notify" data-done="load->replace:#project-develop-target"><i class="icon -cancel -gray"></i></a>';
				$menu .= '</nav>';
			}
			unset($row);
			$row = array(
	...
	if ($isEdit) {
		$ret .= '<form class="sg-form" method="post" action="'.url('project/proposal/'.$tpid.'/info/target.add').'" data-checkvalid="yes" data-rel="notify" data-done="load->replace:#project-develop-target">';
		$targetTables->rows[] = array(
			...
			'<input class="form-text -fill -numeric -require" type="text" name="targetsize" size="3" placeholder="0" autocomplete="off" /> '
