...
function project_form_m1($self,$topic,$para,$report) {
	...
	$isEdit=$topic->project->project_statuscode==1 && $isTeam;
	...
	if (!$period) {
		...
		if ($periodInfo) {
			...
			$ret.=$retStatus;
			$ret.='</div><!-- container -->';
			//			if ($isEdit && $lastPeriodLock>=_PROJECT_LOCKREPORT) $ret.=' หรือ <a class="button" href="'.url('paper/'.$tpid.'/owner/m1/period/'.$nextPeriod,'action=create').'" confirm="ยืนยันการสร้างรายงาน '.$formid.' งวดที่ '.$nextPeriod.' ?">สร้างรายงาน '.$formid.' งวดที่ '.$nextPeriod.'</a>';

			if ($isEdit && $lastPeriodLock>=_PROJECT_LOCKREPORT && $nextPeriod<cfg('project.period.max')) $ret.=' <p><a class="sg-action btn -primary" href="'.url('paper/'.$tpid.'/owner/m1/period/'.$nextPeriod,'action=create').'" data-rel="#main" data-confirm="ต้องการสร้างรายงาน '.$formid.' งวดที่ '.$nextPeriod.' กรุณายืนยัน?"><i class="icon -addbig -white"></i><span>สร้างรายงาน '.$formid.' งวดที่ '.$nextPeriod.'</span></a></p>';
