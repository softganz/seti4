...
function project_form_activity($self, $topic = NULL, $para = NULL, $body = NULL, $part='owner') {
	...
	foreach ($dbs->items as $crs) {
		...
		$ui->add('<a href="'.url('project/join/'.$tpid.'/'.$crs->calid).'">จัดการลงทะเบียน</a>');
...
function __project_activity_form_activity($tpid,$post) {
	...
	if ($part=='owner') {
		$form->text6->type='textarea';
		$form->text6->label='ข้อเสนอแนะต่อ '.cfg('project.grantby').' (ระบุเป็นข้อ)';
		$form->text6->class='-fill';
		$form->text6->cols=15;
		$form->text6->rows=3;
		$form->text6->require=true;
		$form->text6->placeholder='ระบุข้อเสนอแนะต่อ '.cfg('project.grantby').' (ระบุเป็นข้อ)';
		$form->text6->value=$post->text6;

		$form->text7->type='textarea';
		$form->text7->label='ความต้องการสนับสนุนจากพี่เลี้ยงและ '.cfg('project.grantpass').' (ระบุเป็นข้อ)';
		$form->text7->class='-fill';
		$form->text7->cols=15;
		$form->text7->rows=3;
		$form->text7->require=true;
		$form->text7->placeholder='ระบุความต้องการสนับสนุนจากพี่เลี้ยงและ '.cfg('project.grantpass').' (ระบุเป็นข้อ)';
		$form->text7->value=$post->text7;
	} else {
		$form->text7->type='textarea';
		$form->text7->label='ข้อเสนอแนะต่อพื้นที่';
		$form->text7->class='-fill';
		$form->text7->cols=15;
		$form->text7->rows=3;
		$form->text7->require=true;
		$form->text7->placeholder='ระบุข้อเสนอแนะต่อพื้นที่ (ระบุเป็นข้อ)';
		$form->text7->value=$post->text7;

		$form->text6->type='textarea';
		$form->text6->label='ข้อเสนอแนะต่อ '.cfg('project.grantby').'';
		$form->text6->class='-fill';
		$form->text6->cols=15;
		$form->text6->rows=3;
		$form->text6->require=true;
	...
	if ($post->part=='owner') {
		...
		if ($post->flag==_PROJECT_LOCKREPORT) {
			$tables->rows[]=array($post->num1.'<input size="10"  name="activity[num1]" id="edit-activity-num1" class="form-text require" type="hidden" readonly="readonly" value="'.htmlspecialchars($post->num1).'" />',
			...
			$tables->rows[]=array(
												'<input size="10"  name="activity[num1]" id="edit-activity-num1" class="form-text require" type="text" value="'.htmlspecialchars($post->num1).'" placeholder="0.00" />',
												'<input size="10"  name="activity[num2]" id="edit-activity-num2" class="form-text require" type="text" value="'.htmlspecialchars($post->num2).'" placeholder="0.00" />',
												'<input size="10"  name="activity[num3]" id="edit-activity-num3" class="form-text require" type="text" value="'.htmlspecialchars($post->num3).'" placeholder="0.00" />',
												'<input size="10"  name="activity[num4]" id="edit-activity-num4" class="form-text require" type="text" value="'.htmlspecialchars($post->num4).'" placeholder="0.00" />',
												'<input size="10"  name="activity[num5]" id="edit-activity-num5" class="form-text require" type="text" value="'.htmlspecialchars($post->num5).'" placeholder="0.00" />',
												'<input size="10"  name="activity[num6]" id="edit-activity-num6" class="form-text require" type="text" value="'.htmlspecialchars($post->num6).'" placeholder="0.00" />',
												'<input size="10"  name="activity[num7]" id="edit-activity-num7" class="form-text require" type="text" value="'.htmlspecialchars($post->num7).'" placeholder="0.00" />',
											);
		}
	...
	$form->submit->items->save='บันทึกกิจกรรม';
	...
	*/
		...
		$form.submit(function() {
			...
			if (error) {
				...
				console.log($form.attr("autosave"))
				//formSubmit = true;
				//$form.submit()
...
function __project_activity_update() {
