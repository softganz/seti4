<?php
/**
* Project :: Follow Docs Information
* Created 2021-07-28
* Modify  2021-10-27
*
...
class ProjectInfoDocs extends Page {
	var $projectId;
	var $projectInfo;
	...
	function build() {
		...
		$docDb = mydb::select(
			...
			WHERE `tpid` = :projectId AND `type` = "doc" AND `tagname` = "project-docs" AND (`cid` IS NULL OR `cid` = 0)
			ORDER BY `fid`',
		...
		return new Scaffold([
			...
			'body' => new Container([
				...
				'children' => [
						...
						'children' => array_map(function($item) {
							...
							if ($this->right->edit) {
								if ((strtotime($item->timestamp) > strtotime('-30 day')) || $this->right->admin) {
									$ui->add('<a class="sg-action" href="'.url('project/'.$this->projectId.'/info/docs.delete/'.$item->fid).'" data-rel="notify" data-done="remove:parent tr" data-title="ลบไฟล์" data-confirm="ต้องการลบไฟล์นี้ กรุณายืนยัน?"><i class="icon -material -gray">cancel</i></a>');
								} else {
					...
					// new DebugMsg($docDb, '$docDb'),

					$this->right->edit ? new Form([
						'variable' => 'document',
						'action' => url('project/'.$this->projectId.'/info/docs.upload'),
						'id' => 'project-edit-doc',
						'class' => 'sg-form -upload -no-print project-form-upload-docs',
						'enctype' => 'multipart/form-data',
						'rel' => 'notify',
						'done' => 'load->replace:#project-info-docs:'.url('project/'.$this->projectId.'/info.docs'),
						'children' => [
							...
							'title' => [
								...
								'options' => (function(){
									...
									foreach(explode(',', cfg('project')->follow->docsUse) as $docKey => $docTitle) {
