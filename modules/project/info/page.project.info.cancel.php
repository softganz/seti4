...
* @return String
*/
function project_info_cancel($self,$projectInfo) {
	$tpid=$projectInfo->tpid;

	$confirm=SG\getFirst($para->confirm,post('confirm'));
	...
	if (post('cancelreport')) {
		...
		mydb::query('UPDATE %project% SET `project_status`="ยุติโครงการ" WHERE `tpid`=:tpid LIMIT 1',':tpid',$tpid);
		$ret.='<p class="notify">ได้ดำเนินการแจ้งยุติโครงการให้ผู้ดูแลระบบเรียบร้อยแล้ว หากต้องการยกเลิกการแจ้ง กรุณาคลิก <a href="'.url('project/'.$tpid.'/info.cancel',array('cancelreport' => 'yes')).'">ยกเลิกการแจ้งยุติโครงการ</a>';
	} else {
		...
		$form->addField(
			...
			array(
				'type' => 'radio',
				'label' => 'คุณต้องการแจ้งยุติโครงการ <strong>"'.$projectInfo->title.'"</strong>  ใช่หรือไม่?',
				'options' => array('no' => 'ไม่ ฉันไม่ต้องการแจ้ง','yes' => 'ใช่ ฉันต้องการแจ้งยุติโครงการ')
			)
		);
		...
		$form->addField(
			...
			array(
				'type' => 'button',
				'value' => '<i class="icon -material">done_all</i><span>แจ้งยุติโครงการ</span>',
