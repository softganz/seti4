<?php
/**
* Project :: Follow Information API
* Created 2021-07-28
* Modify  2021-09-28
*
* @param Object $self
...
* @return String
*
* @usage project/info/api/{id}/{action}[/{tranId}]
*/
...
class ProjectInfoApi extends Page {
	var $projectId;
	var $action;
	var $tranId;
	...
	function build() {
		...
		$projectInfo = ProjectFollowModel::get($this->projectId, '{initTemplate: true, debug: false'.($isProjectAllType ? ', type: "*"' : '').'}');
		$this->projectId = $projectId = $projectInfo->projectId;
		$tranId = $this->tranId;

		if (!$this->projectId) return 'ERROR : NO PROJECT';
		...
		if (i()->ok && in_array($this->action, $publicApi)) return $this->publicApi();
		...
		switch ($this->action) {
			...
			case 'action.post':
				...
				if (post('calid')) {
					...
					if (empty($calendar)) {
						...
						// Activity no Action
						$data->calid = $calendar->calid;
						$data->activityId = $calendar->activityId;
						$data->part = 'owner';
						$data->trid = $calendar->actionId;
						$data->title = $calendar->title;
						$data->action_date = $calendar->from_date;
						$data->budget = $calendar->budget;
						$ret .= R::View('project.action.form', $projectInfo, $data->activityId, $data, $options);
						//$ret.=print_o($data,'$data');
					}
				} else if ($tranId) {
			...
			case 'photo.upload':
				...
				$data = (Object) [
					'tpid' => $projectId,
					'prename' => 'project_'.$projectId.($post->tagname ? '_'.$post->tagname : '').'_',
					'tagname' => 'project'.($post->tagname ? ','.$post->tagname : ''),
					'title' => $post->title,
					'orgid' => $projectInfo->orgid,
					'refid' => $tranId,
					'cid' => SG\getFirst($post->cid),
					'deleteurl' => $post->delete == 'none' ? NULL : 'project/'.$projectId.'/info/photo.delete/',
					'link' => $post->link,
				];
			...
			case 'expense.photo.upload':
				$data->tpid = $projectId;
				$data->prename = 'project_rcv_'.$projectId.'_';
				$data->tagname = 'project,rcv';
				$data->title = 'เอกสารการเงิน ';
			...
			case 'member.remove':
				if ($tranId && SG\confirm()) {
					mydb::query(
						'DELETE FROM %topic_user% WHERE `tpid` = :projectId AND `uid` = :userId LIMIT 1',
						[':projectId' => $projectId, ':userId' => $tranId]
					);
				}
				break;
			...
			case 'article.add':
				if ($title = post('title')) {
					mydb::query(
						...
						(`tpid`, `uid`, `formId`, `part`, `detail1`, `created`)
						VALUES
						(:projectId, :uid, :formId, :part, :title, :created)
						',
						[
							':projectId' => $projectId,
							':uid' => i()->uid,
					...
					$articleId = mydb()->insert_id;
					location('project/'.$projectId.'/info.article/'.$articleId);
				} else {
					return new ErrorMessage(['code' => _HTTP_ERROR_NOT_ACCEPTABLE, 'text' => 'รายละเอียดไม่ครบถ้วน']);
				}
				break;
			...
			case 'research.add':
				if ($title = post('title')) {
					...
					debugMsg(mydb()->_query);
					$articleId = mydb()->insert_id;
					debugMsg('articleId = '.$articleId);
					location('project/'.$projectId.'/info.research/'.$articleId);
				} else {
					return new ErrorMessage(['code' => _HTTP_ERROR_NOT_ACCEPTABLE, 'text' => 'รายละเอียดไม่ครบถ้วน']);
				}
				break;
			...
			case 'approve.learn':
				mydb::query(
					'UPDATE %topic% SET `approve` = "LEARN" WHERE `tpid` = :projectId LIMIT 1',
					[':projectId' => $projectId]
				);
				$ret = ReactionModel::get(['id' => $projectId, 'bookmark' => 'PROJ.BOOKM']);
				break;

			case 'approve.use':
				mydb::query(
					'UPDATE %topic% SET `approve` = "USE" WHERE `tpid` = :projectId LIMIT 1',
					[':projectId' => $projectId]
				);
				$ret = ReactionModel::get(['id' => $projectId, 'bookmark' => 'PROJ.BOOKM']);
				break;

			case 'financial.period.create':
				if ($period = $tranId) {
					import('model:project.follow.php');
					$periodInfo = ProjectFollowModel::getPeriod($projectId,$period);
					if ($periodInfo && $periodInfo->fromDate && $periodInfo->toDate && $periodInfo->budget) {

						mydb::query(
							'UPDATE %project_tr%
							SET `flag` = :flag, `detail1` = `date1`, `detail2` = `date2`
							WHERE `tpid`=:tpid AND `formid`="info" AND `part`="period" AND `period`=:period
							LIMIT 1',
							[
								':tpid' => $projectId,
								':period' => $period,
								':flag' => _PROJECT_DRAFTREPORT,
							]
						);
						// debugMsg(mydb()->_query);
						...
						mydb::query(
							'INSERT INTO %project_tr%
							SET `tpid`=:tpid, `formid`="ง.1", `part`="summary", `period`=:period, `uid`=:uid, `num1`=0, `num2`=0, `num3`=0, `num4`=0, `num5`=0, `num6`=0, `created`=:created',
							[
								':tpid' => $projectId,
								':period' => $period,
								':uid' => i()->uid,
						...
						return new ErrorMessage(['code' => _HTTP_ERROR_NOT_ACCEPTABLE, 'text' => 'ยังไม่มีการกำหนด <strong>งวดที่ '.$period.'</strong> ในรายละเอียดโครงการ หรือ <strong>ป้อนรายละเอียดของงวดไม่ครบถ้วน</strong> เช่น วันเริ่มงวด,วันสิ้นสุดงวด,งบประมาณ<br /> กรุณา <a href="'.url('project/'.$projectId).'">กำหนดงวดที่ '.$period.' และรายละเอียดให้ครบถ้วนก่อน</a>']);
					}
				}
				break;

			case 'financial.period.delete' :
				if (($period = $tranId) && SG\confirm()) {
					$periodInfo = ProjectFollowModel::getPeriod($projectId, $period);
					if ($isAdmin && $periodInfo->flag == _PROJECT_DRAFTREPORT) {
						$ret .= 'Delete financial report';
						mydb::query(
							'DELETE FROM %project_tr%
							WHERE `tpid` = :projectId AND `formid`="ง.1" AND `period` = :period',
							[
								':projectId' => $projectId,
								':period' => $period,
							]
						...
						mydb::query(
							'UPDATE %project_tr%
							SET `flag` = NULL, `detail1` = NULL, `detail2` = NULL
							WHERE `tpid`=:projectId AND `formid` = "info" AND `part` = "period" AND `period` = :period
							LIMIT 1',
							[
								':projectId' => $projectId,
								':period' => $period,
			...
			case 'progress.create' :
				if ($period = $tranId) {
					$formId = 'ส.1';
					$currentReport = mydb::select(
						'SELECT *
						FROM %project_tr%
						WHERE `tpid` = :projectId AND `formId` = :formId AND `part`="title" AND `period` = :period
						LIMIT 1',
						[
							':projectId' => $projectId,
							':formId' => $formId,
					...
					if ($currentReport->_empty) {
						mydb::query(
							'INSERT INTO %project_tr%
							(`tpid`, `formId`, `period`, `part`, `detail1`, `uid`, `created`)
							VALUES
							(:projectId, :formId, :period, :part, :sendDate, :uid, :created)',
							[
								':projectId' => $projectId,
								':formId' => $formId,
								':period' => $period,
			...
			case 'progress.delete' :
				if (($period = $tranId) && SG\confirm()) {
					mydb::query(
						'DELETE FROM %project_tr%
						WHERE `tpid` = :projectId AND `formId` = "ส.1" AND `period` = :period',
						[
							':projectId' => $projectId,
							':period' => $period,
						]
			...
			case 'approve.master':
				mydb::query(
					'UPDATE %topic% SET `approve` = "MASTER" WHERE `tpid` = :projectId LIMIT 1',
					[':projectId' => $projectId]
				);
				$ret = ReactionModel::get(['id' => $projectId, 'bookmark' => 'PROJ.BOOKM']);
				break;
