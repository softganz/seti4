...
class ProjectInfoShare extends Page {
	var $projectId;
	var $projectInfo;
	...
	function build() {
		if (!$this->projectId) {
			return  new ErrorMessage(['responseCode' => _HTTP_OK_NO_CONTENT, 'text' => 'ไม่มีข้อมูลข้อเสนอโครงการที่ระบุ']);
		}

		$topicMember = i()->ok ? R::Model('paper.membership.get',$this->projectId,i()->uid) : NULL;
		$orgMember = $this->projectInfo->orgId && i()->ok ? OrgModel::officerType($this->projectInfo->orgId,i()->uid) : NULL;
		...
		return new Scaffold([
			...
			'body' => new Container([
				'id' => 'project-info-share',
				'attribute' => ['data-url' => url('project/'.$this->projectId.'/info.share')],
				'children' => [
					new NodeShareWidget([
						'copyLink' => _DOMAIN.url('project/'.$this->projectId),
						...
						'members' => new Table([
							'id' => 'project-info-member-list',
							'class' => 'project-info-member-list',
							'showHeader' => false,
