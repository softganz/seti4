...
class ProjectInfoDashboard extends Page {
	...
	function build() {
		...
		if ($isAdmin) {
			$adminUi = new Ui(NULL, '-project-dashboard');
			$adminUi->addConfig('nav', '{class: "nav -app-menu"}');

			$adminUi->header('<h3>Admin</h3>');
			$adminUi->add('<a class="sg-action" href="'.url('project/'.$projectId.'/page.setting').'" data-rel="box" data-width="full"><i class="icon -material">settings</i><span>กำหนดค่า</span></a>');
			//$ui->add('<a href="'.url('paper/'.$projectId.'/edit').'"><i class="icon -material">settings</i><span>จัดการหัวข้อ</span></a>');
			if ($isDeletable) {
				$adminUi->add('<a class="sg-action" href="'.url('project/'.$projectId.'/info.delete').'" data-rel="box" data-width="640"><i class="icon -material">delete</i><span>ลบโครงการ</span></a>');
			}
			$ret .= $adminUi->build('ul');
		}
