...
function project_org_view($self, $orgId = NULL) {
	...
	$tables = new Table();
	$tables->thead=array('amt -year'=>'ปี','ชื่อโครงการ','amt -budget'=>'งบประมาณ','amt -act'=>'กิจกรรม');
	$no=0;
	...
	$ret .= $tables->build();
	$ret .= '<nav class="nav -page -sg-text-right">จำนวน '.$dbs->_num_rows.' โครงการ ';

	$ret .= '<a class="btn -link" href="'.url('project/org/'.$orgInfo->orgid.'/follow').'"><i class="icon -list"></i><span>โครงการทั้งหมด</span></a></nav>';
	$ret .= '</div>';
	...
	$stmt='SELECT u.`uid`, u.`username`, u.`name`, UPPER(tu.`membership`) `membership`
				...
				WHERE tu.`orgid`=:orgid AND u.`status`="enable" AND tu.`membership` NOT IN ("MEMBER","REGULAR MEMBER")
	...
	foreach ($member->items as $mrs) {
		$tables->rows[]=array(
			...
			($isViewMemberProfile?'<span><a class="sg-action" href="'.url('project/org/'.$orgId.'/trainer/'.$mrs->uid).'" data-rel="box">':'').$mrs->name.($isViewMemberProfile?'</a></span>':''),
			$mrs->membership,
			$isEditTrainer && in_array($mrs->membership,array('TRAINER','MEMBER')) ?'<a class="sg-action" href="'.url('project/org/'.$orgId.'/removemember/'.$mrs->uid).'" title="ลบ" data-rel="none" data-removeparent="tr" data-confirm="ต้องการลบข้อมูล กรุณายืนยัน?"><i class="icon -cancel"></i></a> ':''
			);
	}
	$ret .= $tables->build();
