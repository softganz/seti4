...
function view_project_my_action_render($projectInfo=NULL,$actionInfo=NULL,$action=NULL) {
	...
	if ($isItemEdit) {
		...
		$ret.='<nav class="nav -card">'.sg_dropbox($ui->build(),'{icon:"material -gray",class:"leftside -atright"}').'</nav>';
	}
	...
	if ($actionInfo->gallery || $actionInfo->rcvPhotos) {
		$photos = mydb::select('SELECT f.`fid`, f.`type`, f.`file`, f.`title`, f.`tagname` FROM %topic_files% f WHERE f.`tpid` = :tpid AND (f.`gallery` = :gallery OR (f.`refid` = :refid AND `tagname` = :tagname))', ':tpid', $tpid, ':refid', $actionId, ':tagname', 'project,rcv', ':gallery', SG\getFirst($actionInfo->gallery,-1));
	...
	if ($isItemEdit) {
		$ret.='<div style="margin:20px 0;">'._NL.'<form class="sg-upload" method="post" enctype="multipart/form-data" action="'.url('project/'.$tpid.'/info/expense.photo.upload/'.$actionId).'" data-rel="#project-rcvphoto-'.$actionId.'" data-append="li"><span class="btn btn-success fileinput-button"><i class="icon -camera"></i><span>ส่งภาพใบเสร็จรับเงิน</span><input type="file" name="photo[]" multiple="true" class="inline-upload -rcv" /></span></form>'._NL.'</div>'._NL;
	}
	$ret.='</div><!-- photo -->';
	...
	$tables->thead='<thead><tr><th colspan="6">ประเภทรายจ่าย</th><th rowspan="2">รวมรายจ่าย</th><th rowspan="2">สถานะ</th></tr><tr><th>ค่าตอบแทน</th><th>ค่าจ้าง</th><th>ค่าใช้สอย</th><th>ค่าวัสดุ</th><th>ค่าสาธารณูปโภค</th><th>อื่น ๆ</th></tr></thead>';
	$tables->rows[]=array(
										number_format($actionInfo->exp_meed,2),
										number_format($actionInfo->exp_wage,2),
										number_format($actionInfo->exp_supply,2),
