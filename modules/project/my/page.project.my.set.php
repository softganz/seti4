 ...
 * @return String
 */
function project_my_set($self, $action = NULL) {
	R::View('project.toolbar',$self,'ชุดโครงการ','my',$projectInfo,'{modulenav:false}');
	...
	if ($action) {
		switch ($action) {
			case 'new':
				$ret .= R::Page('project.set.new',NULL);
				break;
	...
	*/
	...
	$isCreatePlanning = user_access('create project set')
											&&  in_array('my/set', explode(',', cfg('PROJECT.SET.ADD_FROM_PAGE')));

	if ($isCreatePlanning) {
		$ret.='<nav class="nav btn-floating -right-bottom"><a class="sg-action btn -floating -circle48" href="'.url('project/my/set/new').'" title="Create New Planning"><i class="icon -addbig -white"></i></a></nav>';
	}
