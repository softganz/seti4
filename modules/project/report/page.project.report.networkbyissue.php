...
function project_report_networkbyissue($self, $tpid = NULL) {
	...
	$ret.='<form id="project-develop" method="get" action="'.url('project/report/networkbyissue').'">';
	$ret.='<ul>';
	...
	$stmt='SELECT 
		...
		%WHERE%
		ORDER BY tr.`detail3` ASC, CONVERT(`networkName` USING tis620) ASC';

	$dbs=mydb::select($stmt,$where['value']);


	$tables = new Table();
	$tables->thead=array('no'=>'','ชื่อเครือข่าย','center -c1'=>'ประเภท','ชื่อแกนนำ','สถานที่ติดต่อ','ศักยภาพเครือข่าย');
	$tables->addConfig('showHeader',false);

	$curIssue='';
	$no=0;
	foreach ($dbs->items as $rs) {
		if ($curIssue!=$rs->orgissueName) {
			$tables->rows[]=array('<td colspan="6">ประเด็น : '.$rs->orgissueName.'</td>','config'=>array('class'=>'subheader'));
			$tables->rows[]='<header>';
			$curIssue=$rs->orgissueName;
			$no=0;
		}
		$tables->rows[]=array(
			++$no,
	...
	.item td:nth-child(n+3) {width:17.5%;}
	.item .subheader {font-size:1.3em; background-color:#bbb;}
	</style>';
	return $ret;
}
?>