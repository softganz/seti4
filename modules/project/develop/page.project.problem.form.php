...
function project_problem_form($self,$tpid=NULL) {
	...
	$form=new Form(NULL,url('project/'.$tpid.'/info.problem/edit'),NULL,'sg-form project-develop-problem-form');
	...
	*/
	...
	$form->addField(
				...
				array(
					...
					'posttext'=>'<a class="sg-action" href="'.url('project/'.$tpid.'/info.problem/edit').'" data-rel="#project-develop-problem">{tr:Cancel}</a>',
					)
				);
	$ret.=$form->build();
	$ret.='<style type="text/css">
	.project-develop-problem-form {margin:32px 16px; text-align:left; box-shadow:0 0 0 1px #eee inset;border-radius:4px;}
	.project-develop-problem-form h3.title {border-radius:4px 4px 0 0;font-size:1.2em;background:#e5e5e5;color:#666;text-align:center;}
	.project-develop-problem-form .form-item {padding:8px;}
	</style>';
