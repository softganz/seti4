...
function project_develop_area($self,$tpid,$action=NULL,$trid=NULL) {
	...
	foreach ($dbs->items as $rs) {
		$row=array(
						...
						$rs->tambonName,
						$rs->areatype
						. ($isEdit ? '<nav class="nav iconset -hover"><a class="sg-action" href="'.url('project/develop/area/'.$tpid.'/delete/'.$rs->autoid).'" data-rel="none" data-removeparent="tr" data-confirm="ต้องการลบรายการนี้ กรุณายืนยัน?"><i class="icon -cancel -gray -hover"></i></a></nav>' : ''),
						);
		$tables->rows[]=$row;
	}
