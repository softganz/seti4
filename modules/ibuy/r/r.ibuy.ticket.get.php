...
function r_ibuy_ticket_get($conditions, $options = '{}') {
	$defaults = '{debug: false, includeThread: false, order: "`tickid` DESC", limit: 1, start: -1}';
	...
	if ($conditions->customerId) mydb::where('tk.`custid` = :custid AND tk.`thread` IS NULL', ':custid', $conditions->customerId);

	if ($conditions->serialId) mydb::where('tk.`issnid` = :issnid', ':issnid', $conditions->serialId);
	...
	$stmt = 'SELECT
		...
		FROM %ticket% tk
			LEFT JOIN %ibuy_customer% c USING(`custid`)
			LEFT JOIN %ticket% th ON th.`tickid` = tk.`thread`
