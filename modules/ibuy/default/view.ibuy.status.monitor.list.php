 ...
 * @return String
 */
function view_ibuy_status_monitor_list() {
	$isAdmin=user_access('administer ibuys');
	if (!$isAdmin) return;
	$stmt='SELECT l.lid,l.kid,l.created,l.amt,l.uid,f.custname,l.detail, o.`orderno`
					FROM %ibuy_log% l
	...
	if ($dbs->_num_rows) {
		...
		$tables->header=array('คำสั่งซื้อ','date'=>'Log Date','ลูกค้า','Transfer Date - Time','money'=>'Amout','Action');
	...
	*/
	...
	$tables->header=array('คำสั่งซื้อ','date'=>'วันที่สั่งซื้อ','','ร้านค้า','T','money subtotal'=>tr('Subtotal'),'money discount'=>tr('Discount'),'money shipping'=>tr('Shipping'),'money total'=>tr('Total'),'money balance'=>'ค้างชำระ','ขนส่งโดย','Action','status'=>tr('Status'),'','');
	foreach ($orders->items as $rs) {
		...
		$tables->rows[]=array($rs->orderno,
											...
											$isAdmin /** && $rs->status==40 **/?'<a href="'.url('ibuy/status/closeorder/'.$rs->oid).'" class="sg-action" data-confirm="ปิดสถานะใบสั่งสินค้า กรุณายืนยัน" data-removeparent="tr" data-rel="none" title="ปิดสถานะใบสั่งสินค้า"><i class="icon -unlock"></i></a>':'',
											'config'=>array('class'=>'status-'.$rs->status)
										);
		if ($rs->remark) $tables->rows[]='<tr><td colspan="2"></td><td colspan="11">'.($rs->remark?'หมายเหตุ : '.$rs->remark:'').'</td></tr>';
	}
	$ret .= $tables->build();
	return $ret;
}
