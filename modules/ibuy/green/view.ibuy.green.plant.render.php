...
function view_ibuy_green_plant_render($treeInfo, $options = '{}') {
	$ret = '';
	...
	*/
	...
	if ($isItemEdit) {
		$dropUi->add('<a class="sg-action" href="'.url('ibuy/my/samtree/form/'.$treeInfo->plantid).'" data-rel="box" data-width="640"><i class="icon -material">edit</i><span>แก้ไขรายละเอียด</span></a>');
		$dropUi->add('<a class="sg-action" href="'.url('ibuy/my/info/activity.delete/'.$treeInfo->msgid).'" data-rel="notify" data-done="remove:parent .ui-card>.ui-item" data-title="ลบพืชแซมยาง" data-confirm="ต้องการลบพืชแซมยาง กรุณายืนยัน?"><i class="icon -material">delete</i><span>ลบพืชแซมยาง</span></a>');
	}
	...
	$ret = '<div class="header">'._NL
		...
		. '<h3>'.$treeInfo->productname.'@'.$treeInfo->landName.'</h3>'._NL
	...
	if ($treeInfo->round) {
		$stmt = 'SELECT * FROM %bigdata% WHERE `keyname` = "ibuy.tree" AND `keyid` = :plantid';
	...
	// Show Tree Photo
	$stmt = 'SELECT * FROM %topic_files% WHERE `tagname` = :tagname AND `refid` = :refid';
	$photoDbs = mydb::select($stmt, ':refid', $treeInfo->msgid, ':tagname', $treeInfo->tagname);
	...
	list($treeModule, $treeTagName) = explode(',', $treeInfo->tagname);
	if ($isItemEdit) {
		$navUi->add(
