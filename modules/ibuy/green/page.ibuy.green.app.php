<?php
/**
* iBuy Green Smile App Home Pahe
* Created 2019-10-27
* Modify  2019-10-27
*
* @param Object $self
...
function ibuy_green_app($self) {
	$isDevVersion = true; //in_array(i()->username, explode(',',cfg('ibuy.useDevVersion')));
	$isShowAll = true;
...
/*
	...
	if (is_admin()) {
		$ret .= '<a class="btn" href="'.url('ibuy/green/walkin/list').'">รายการลงทะเบียน</a>';
	}
	$ret .= '</div>';
	*/

	//$ret .= '<div class="sg-load" data-url="'.url('ibuy/green/news').'"></div>';

	if ($isShowAll) $ret .= R::Page('ibuy.green.news', NULL);


	// Show main navigator
	$ui = new Ui();
	$ui->add('<a class="sg-action" href="'.url('ibuy/green/shop').'" data-webview="ร้านค้า"><i class="icon -material">account_balance</i><span>ร้านค้า</span></a>');
	$ui->add('<a class="sg-action" href="'.url('ibuy/green/goods').'" data-webview="สินค้า"><i class="icon -material">local_florist</i><span>สินค้า</span></a>');
	$ui->add('<a class="sg-action" href="'.url('ibuy/green/plant').'" data-webview="จองผลผลิต"><i class="icon -material">done_all</i><span>จอง</span></a>');
	$ui->add('<a class="sg-action" href="'.url('ibuy/green/follow').'" data-webview="ติดตาม"><i class="icon -material">star</i><span>ติดตาม</span></a>');
	$ui->add('<a class="sg-action" href="'.url('ibuy/green/category').'" data-webview="หมวด"><i class="icon -material">view_module</i><span>หมวด</span></a>');

	if ($isShowAll) $ret .= '<nav class="nav -page -app-icon">'.$ui->build().'</nav>'._NL;


	// Show Activity Post Button
	if ($isDevVersion) {
		$ret .= '<div id="ibuy-green-chat-box" class="ui-card ibuy-green-chat-box">'
			. '<div class="ui-item">'
			. '<header class="header"><h3>เขียนบันทึก</h3></header>'
			. '<div><img src="'.model::user_photo(i()->username).'" width="40" height="40" />&nbsp;'
			. '<a class="sg-action form-text" href="'.url('ibuy/green/activity/form').'" placeholder="เขียนบันทึกการทำกิจกรรม" data-rel="box" data-width="480" data-height="640" data-webview="บันทึกการทำกิจกรรม">เขียนบันทึกการทำกิจกรรม</a>&nbsp;'
			. '<a class="sg-action btn -link" href="'.url('ibuy/green/activity/form').'" data-rel="box" data-width="480" data-height="640" data-webview="บันทึกการทำกิจกรรม"><i class="icon -camera"></i><span>Photo</span></a></div>'
			. '</div>'
			. '</div>';
	}
	...
	//$ret .= R::Page('ibuy.green.activity', NULL);

	// Show visit history
	$ret .= '<section id="ibuy-green-activity-card" class="sg-load" data-url="'.url('ibuy/green/activity', array('u' => post('u'))).'">'._NL;
	...
	if ($isShowAll) {
		...
		// Show Green Goods
		$ret .= '<div class="sg-load" data-url="'.url('ibuy/green/goods').'"></div>';
	...
	@media (min-width:30em){ /* 480/16 = 30 */
		...
		.ibuy-green-chat-box {margin: 16px 0;}
	}
	</style>');
	...
	$(document).on("click", ".btn.-ibuy-plant-down", function() {
	...
	$(document).on("click", ".btn.-ibuy-plant-up", function() {
		var $qty = $(this).closest(".form-item").find(".form-text")
	...
	if (cfg('firebase')) {
		...
		$(document).ready(function() {
			...
			var drawUrl = "'.url('ibuy/green/activity/render').'/"
			var i = 0
			...
			.on("child_added",function(snap){
				if (snap.val().changed == "new") {
					$.post(drawUrl + snap.val().seq, function(html) {
						console.log(html)
						if (html) {
							var visitBox = $("#ibuy-green-activity")
