...
function view_ibuy_green_activity_comment_render($commentInfo, $options = '{}') {
	...
	$dropUi = new Ui();
	if ($isEdit) {
		$dropUi->add('<a class="sg-action" href="'.url('ibuy/my/info/activity.comment.delete/'.$commentInfo->msgid).'" title="ลบข้อความ" data-rel="none" data-title="ลบข้อความ" data-confirm="ลบข้อความนี้ทิ้ง กรุณายืนยัน?" data-done="remove:parent .ui-card>.ui-item"><i class="icon -delete"></i><span>ลบข้อความ</span></a>');
	}

	if ($dropUi->count()) $headerUi->add(sg_dropbox($dropUi->build()));

	$posterUrl = $options->page == 'app' ? '<a class="sg-action" href="'.url('ibuy/green/u/'.$commentInfo->uid, array('ref' => 'app')).'" data-rel="box" data-webview="'.$commentInfo->posterName.'" data-width="480" data-height="80%">' : '<a class="sg-action" href="'.url('ibuy/green/u/'.$commentInfo->uid).'" data-rel="box" data-done="moveto:0,0" data-width="480" data-height="80%">';


	// Create Photo Album
	$photoAlbum = new Ui(NULL, 'ui-album -justify-left');
	$photoAlbum->addId('ibuy-activity-photo-'.$commentInfo->msgid);
	...
	if ($commentInfo->photoList) {
		foreach (explode(',',$commentInfo->photoList) as $photoItem) {
			...
			if ($isEdit) {
				$Ui->add('<a class="sg-action -no-print" href="'.url('ibuy/my/info/activity.photo.delete/'.$commentInfo->msgid,array('fid' => $fid)).'" title="ลบภาพนี้" data-confirm="ยืนยันว่าจะลบภาพนี้จริง?" data-rel="notify" data-removeparent="li"><i class="icon -material">cancel</i></a>');
			}
	...
	if ($isOwner) {
		$photoAlbum->add(
			'<form class="sg-upload" method="post" enctype="multipart/form-data" action="'.url('ibuy/my/info/activity.photo.upload/'.$commentInfo->msgid).'" data-rel="#ibuy-activity-photo-'.$commentInfo->msgid.'" data-before="li">'
	...
	*/
	...
	if (i()->ok) {
		$cardUi->add('<a class="sg-action btn -link" href="'.url('underconstruction/1/ibuy/msg/like/'.$commentInfo->msgid).'" data-rel="box" data-width="640" data-width="80%"><i class="icon -material">thumb_up</i><span>Like</span></a>');
		$cardUi->add('<a class="sg-action btn -link" href="'.url('underconstruction/1/ibuy/msg/comment/'.$commentInfo->msgid).'" data-rel="box" data-width="640" data-width="80%"><i class="icon -material">comment</i><span>Comment</span></a>');
		$cardUi->add('<a class="sg-action btn -link" href="'.url('underconstruction/1/ibuy/msg/share/'.$commentInfo->msgid).'" data-rel="box" data-width="640" data-width="80%"><i class="icon -material">share</i><span>Share</span></a>');
	}
	...
	*/
	...
	if (i()->ok) {
		$ret .= '<div class="-comment-form">'
			. '<form class="form sg-form -sg-flex" action="'.url('ibuy/my/info/activity.comment.save/'.$commentInfo->msgid).'" data-rel="notify">'
