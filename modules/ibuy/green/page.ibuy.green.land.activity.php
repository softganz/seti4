...
function ibuy_green_land_activity($self, $landInfo) {
	...
	if ($dbs->_num_rows && $dbs->_num_rows == $showItems) {
		$ret .= '<div id="more" class="ibuy-green-activity-more" style="padding: 24px 16px 44px;">'
			. '<a class="sg-action btn -primary" href="'.url('ibuy/green/land/'.$landId.'/activity',array('start' => $start+$dbs->_num_rows)).'" data-rel="replace:#more" style="margin:0 auto;display:block;text-align:center; padding: 16px 0;">'
			. '<span>{tr:More}</span>'
