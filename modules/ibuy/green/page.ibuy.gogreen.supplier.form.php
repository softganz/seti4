<?php
function ibuy_green_supplier_form($self,$qtref=NULL) {
	...
	if ( empty($qtref) && $data->{'qt:ORG.NAME'} )  {
		// Create new qt
		$data->appsrc='Android';
		$result=R::Model('ibuy.green.supplier.save',$data);
		//$ret.='Create new';
		//$ret.=print_o($data,'$data');
		...
		//	$ret.=print_o($result,'$result');
			...
			location('ibuy/green/supplier/form/'.$result->qtref);
		}
	}
	...
	if (empty($qtref)) {
		...
		$form=new Form('data',url('ibuy/green/supplier/form'),'imed-poorman-form','sg-form imed-poorman-form');
		...
		$form->addField('save',
							array(
								...
								'items'=>array(
													'type'=>'submit',
													'class'=>'-primary',
													'value'=>'<i class="icon -save -white"></i><span>ลงทะเบียนเครือข่าย</span>',
													),
								)
							);
		...
		foreach ($regDbs->items as $rs) {
			if ($rs->orgid) {
				$url=url('ibuy/green/supplier/'.$rs->qtref.'/view');
			} else {
				$url=url('ibuy/green/supplier/form/'.$rs->qtref);
			}
	...
	if (post('save')) {
		...
		// $data->deleteurl=url('imed/visit/'.$data->psnid.'/deletephoto/');
	...
	// Get quatation data
	$qtInfo=R::Model('ibuy.green.qt.get',$qtref);


	// Show full quatation form
	R::View('org.toolbar',$self,$qtInfo->name,'supplier');
	$ret.=R::View('ibuy.green.supplier.form.'.$qtInfo->qtform,$qtInfo);
...
function __imed_app_poorman_form_photo($photoList,$tagname) {
	...
	foreach ($photoList as $rs) {
		...
		if ($isEditable) {
			...
			// $ui->add('<a class="sg-action -no-print" href="'.url('imed/visit/'.$rs->psnid.'/deletephoto/'.$rs->fid).'" title="ลบภาพนี้" data-confirm="ยืนยันว่าจะลบภาพนี้จริง?" data-rel="none" data-removeparent="li"><i class="icon -delete"></i></a>');
			$ret.=$ui->build();
		}
		$ret.='</li>'._NL;
	}
