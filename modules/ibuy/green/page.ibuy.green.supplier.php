<?php
function ibuy_green_app_supplier($self,$qtref=NULL,$action=NULL,$trid=NULL) {
	...
	switch ($action) {
		case 'view':
			if ($qtInfo->orgid) {
				$ret.=R::Page('ibuy.green.supplier',NULL,$qtInfo->orgid);
			} else {
		...
		default:
			...
			foreach ($dbs->items as $rs) {
				...
				if ($rs->uid==i()->uid || $isAdmin) {
					$menu.='<a class="btn -primary -circle" href="'.url('ibuy/green/app/supplier/form/'.$rs->qtref).'"><i class="icon -edit -white"></i></a>';
				}
			...
			foreach ($dbs->items as $rs) {
				$menu='';
				if ($rs->uid==i()->uid || $isAdmin) {
					$menu.='<a href="'.url('ibuy/green/app/supplier/form/'.$rs->qtref).'"><i class="icon -edit"></i></a>';
				}
				$tables->rows[]=array(
													'<a href="'.url('ibuy/green/app/supplier/'.$rs->qtref.'/view').'">'.$rs->name.'</a>',
													$menu
													);
			}
			$ret.=$tables->build();
