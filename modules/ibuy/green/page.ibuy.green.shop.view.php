...
function ibuy_green_shop_view($self, $shopInfo) {
	...
	$shopUrl = url('ibuy/green/shop/'.$shopId);
	...
	// Show main navigator
	$ui = new Ui();
	$ui->add('<a class="sg-action" href="'.url('ibuy/green/shop/'.$shopId.'/plant').'" data-webview="สินค้า"><i class="icon -material">nature_people</i><span>ผลผลิต</span></a>');
	...
	$ui->add('<a class="sg-action" href="'.url('ibuy/green/shop/'.$shopId.'/goods').'" data-webview="สินค้า"><i class="icon -material">local_florist</i><span>สินค้า</span></a>');
	$ui->add('<a class="sg-action" href="'.url('ibuy/green/shop/'.$shopId.'/follow').'" data-webview="ติดตาม"><i class="icon -material">star</i><span>ติดตาม</span></a>');
	$ui->add('<a class="sg-action" href="'.url('ibuy/green/shop/'.$shopId.'/contact').'" data-webview="ติดต่อ"><i class="icon -material">people</i><span>ติดต่อ</span></a>');

	$ret .= '<nav class="nav -page -app-icon">'.$ui->build().'</nav>'._NL;

	$ret .= R::Page('ibuy.green.shop.plant', NULL, $shopInfo);


	$ret .= R::Page('ibuy.green.shop.goods', NULL, $shopInfo);
	...
	$qrCode = SG\qrcode(url('ibuy/green/shop/'.$shopId),'{width: 512, height: 512}');
