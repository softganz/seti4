...
function ibuy_green_my_cotree($self, $landId = NULL) {
	...
	R::View('toolbar',$self, 'พืชร่วมยาง @'.$headerTitle,'ibuy.green.my.cotree',$landInfo);
	...
	// Get Tree in my Land
	mydb::where('p.`tagname` = "ibuy-cotree" AND l.`orgid` = :orgid', ':orgid', $shopId);
	...
	$stmt = 'SELECT
		...
		FROM %ibuy_farmplant% p
			...
			LEFT JOIN %msg% m ON m.`tagname` = "ibuy-cotree" AND m.`plantid` = p.`plantid`
