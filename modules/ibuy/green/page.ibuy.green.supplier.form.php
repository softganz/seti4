<?php
function ibuy_green_app_supplier_form($self,$qtref=NULL) {
	...
	if ( empty($qtref) && $data->{'qt:ORG.NAME'} )  {
		...
		//	$ret.=print_o($result,'$result');
			...
			location('ibuy/green/app/supplier/form/'.$result->qtref);
		}
	}
	...
	if (empty($qtref)) {
		...
		$form=new Form('data',url('ibuy/green/app/supplier/form'),'imed-poorman-form','sg-form imed-poorman-form');
		...
		foreach ($regDbs->items as $rs) {
			if ($rs->orgid) {
				$url=url('ibuy/green/app/supplier/'.$rs->qtref.'/view');
			} else {
				$url=url('ibuy/green/app/supplier/form/'.$rs->qtref);
			}
	...
	if (post('save')) {
		$result=R::Model('ibuy.green.supplier.save',$data);
		location('ibuy/green/app/supplier/list');
		return $ret;
	} else if ($_FILES) {
		$photoKey=key($_FILES);
	...
	$ret.=R::View('ibuy.green.app.supplier.form.'.$qtInfo->qtform,$qtInfo);
