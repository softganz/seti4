...
function ibuy_green_goods_view($self, $tpid) {
	$productInfo = R::Model('ibuy.product.get', $tpid, '{debug: false}');

	$ret .= '';
	
	if ($productInfo->orgid) {
		$ret = '<h3><a class="sg-action btn -link" href="'.url('ibuy/green/shop/'.$productInfo->orgid).'" data-webview="'.$productInfo->info->shopName.'">'.$productInfo->info->shopName.'</a></h3>';
	...
	if ($productInfo->orgid) {
		...
		if ($cardUi->count()) {
			...
			$ret .= '<heder class="header"><h3><a class="sg-action btn -link" href="'.url('ibuy/green/shop/'.$productInfo->orgid).'" data-webview="'.$productInfo->info->shopName.'">สินค้าจากร้าน '.$productInfo->info->shopName.'</a></h3></header>';
