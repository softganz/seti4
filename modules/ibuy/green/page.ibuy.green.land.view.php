...
function ibuy_green_land_view($self, $landInfo) {
	if (!($landId = $landInfo->landId)) return message('error', 'PROCESS ERROR');

	$ret = '';

	R::View('toolbar',$self,$landInfo->landName.' @Green Smile','ibuy.green.shop',$landInfo);
	...
	$headerUi->add('<a class="sg-action btn'.($landInfo->info->approved == 'Approve' ? ' -success' : '').'" href="'.url($isAdmin ? 'ibuy/green/standard/set/'.$landInfo->info->landid : 'ibuy/green/standard/info/'.$landInfo->info->landid).'" data-rel="box" data-width="320">'
	...
	if (in_array($landInfo->info->approved, array('Approve','ApproveWithCondition'))) {
		$headerUi->add('<a class="sg-action btn -link" href="'.url('ibuy/green/land/'.$landInfo->info->landid.'/qr').'" data-rel="box" data-width="320" data-webview="QR Code" title="QR Code ของแปลงผลิต"><i class="icon -qrcode -sg-24"></i><span class="-hidden">QR Code</span></a>');
	}
	if ($landInfo->info->location) {
		$headerUi->add('<a class="sg-action btn -link" href="'.url('ibuy/green/land/'.$landInfo->info->landid.'/map', array('options:fullpage,notoolbar'=>'')).'" data-rel="box" data-width="640" data-class-name="-map" data-webview="แผนที่แปลงผลิต" data-refresh="no"><i class="icon -material -land-map -active">where_to_vote</i><span class="-hidden">แผนที่</span></a>');
	}

	if ($isItemEdit) {
		//$dropUi->add('<a class="sg-action" href="'.url('ibuy/my/land/form/'.$landInfo->info->landid).'" data-rel="box" data-width="640"><i class="icon -material">edit</i><span>แก้ไขรายละเอียด</span></a>');
		//$dropUi->add('<a class="sg-action" href="'.url('ibuy/my/info/land.remove/'.$landInfo->info->landid).'" data-rel="notify" data-done="remove:parent .ui-card.-land>.ui-item" data-title="ลบแปลงผลิต" data-confirm="ต้องการลบแปลงผลิต กรุณายืนยัน?"><i class="icon -material">cancel</i><span>ลบแปลงผลิต</span></a>');
	}
	...
	$stmt = 'SELECT * FROM %topic_files% WHERE `orgid` = :orgid AND `tagname` = "ibuy,land" AND `refid` = :refid';
	...
	if ($isItemEdit) {
		$navUi->add('<form class="sg-upload" method="post" enctype="multipart/form-data" action="'.url('ibuy/my/info/photo.upload/'.$landInfo->info->landid).'" data-rel="#ibuy-land-photo-'.$landInfo->info->landid.'" data-append="li"><input type="hidden" name="tagname" value="land" /><span class="btn -link fileinput-button"><i class="icon -material">photo_camera</i><span class="-sg-is-desktop">'.$cameraStr.'</span><input type="file" name="photo[]" multiple="true" class="inline-upload" accept="image/*;capture=camcorder" /></span><input class="-hidden" type="submit" value="upload" /></form>');

		$navUi->add('<a class="sg-action btn -link" href="'.url('ibuy/my/land/map/'.$landInfo->info->landid).'" data-rel="box" data-width="640" data-class-name="-map"><i class="icon -material -land-map '.($landInfo->info->latlng ? '-active' : '').'">'.($landInfo->info->latlng ? 'where_to_vote' : 'room').'</i><span class="-sg-is-desktop">แผนที่</span></a>');

		//$navUi->add('<a class="sg-action btn -link" href="'.url('ibuy/my/land/standard/'.$landInfo->info->landid).'" data-rel="box" data-width="640"><i class="icon -material">how_to_reg</i><span>มาตรฐาน</span></a>');

		$navUi->add('<a class="sg-action btn" href="'.url('ibuy/my/plant/form',array('land' => $landInfo->info->landid)).'" data-rel="box" data-width="640"><i class="icon -material">add_circle_outline</i><span>ผลผลิตรอบใหม่</span></a>');
	}
