<?php
function garage_code_wage($self, $action = NULL, $id = NULL) {
	$shopInfo=R::Model('garage.get.shop');

	new Toolbar($self, 'รหัสค่าแรง');

	switch ($action) {
		case 'edit':
			$data=R::Model('garage.repaircode.get',$id,'{debug:false}');
			//$ret.=print_o($data,'$data');
			$ret.=__garage_code_wage_list($shopInfo,$data,$action,$id);
			break;
		
		case 'save':
			$data=(object)post();
			$saveResult.=__garage_code_wage_save($shopInfo,$data);
			$ret.=__garage_code_wage_list($shopInfo,NULL,NULL,$data->repairid);
			$ret.=$saveResult;
			break;

		case 'delete':
			if ($id && SG\confirm()) $ret.=__garage_code_wage_delete($shopInfo,$id);
			break;

		case 'list':
			$ret.=__garage_code_wage_list($shopInfo);
			break;
...
function __garage_code_wage_list($shopInfo, $data = NULL, $action = NULL, $trid = NULL) {
	$shopid=$shopInfo->shopid;
	$stmt='SELECT r.*, COUNT(j.`repairid`) `idUsed`
				FROM %garage_repaircode% r
					LEFT JOIN %garage_jobtr% j USING(`repairid`)
				WHERE r.`shopid` IN (:shopid) AND r.`repairtype`=3
				GROUP BY `repairid`
				ORDER BY r.`repaircode` ASC';
	$dbs=mydb::select($stmt,':shopid',$shopid);

	$ret.='<form id="garage-job-code-new" class="sg-form" method="post" action="'.url('garage/code/wage/save').'" data-checkvalid="true" data-rel="#garage-code-trans">'._NL;

	$tables = new Table();
	$tables->addClass('-brand');
	$tables->thead=array('รหัสค่าแรง','รายละเอียดค่าแรง','money -A'=>'ราคา A','money -B'=>'ราคา B','money -C'=>'ราคา C','money -D'=>'ราคา D','');

	$tables->rows[]=array(
		'<input type="hidden" name="oldid" value="'.$data->repairid.'" />'
		.'<input id="codeid" class="form-text sg-autocomplete -fill -uppercase -require" type="text" name="repaircode" value="'.$data->repaircode.'" placeholder="รหัสค่าแรง" size="10" maxlength="10" data-query="'.url('garage/api/repaircode').'" />',
		'<input id="repairname" class="form-text sg-autocomplete -fill -require" type="text" name="repairname" value="'.$data->repairname.'" placeholder="รายละเอียดค่าแรง" data-query="'.url('garage/api/repaircode').'" />',
	...
	$tables->rows[]=array(
		'',
		'<input id="repairinsu" class="form-text -fill -hidden" type="hidden" name="repairinsu" value="'.$data->repairinsu.'" placeholder="รายละเอียดค่าแรงสำหรับบริษัทประกัน" />',
		...
		.($action=='edit'?'<a class="sg-action button-cancel" href="'.url('garage/code/wage/list').'" data-rel="#garage-code-trans">ยกเลิกแก้ไข</a>':'')
		.'</td>',
		'',
		'config'=>array('class'=>'-input -no-print'),
	);
	...
	foreach ($dbs->items as $rs) {
		...
		$ui->add('<a class="sg-action" href="'.url('garage/code/wage/edit/'.$rs->repairid).'" data-rel="#garage-code-trans" data-callback="garageCodeEditClick"><i class="icon -edit"></i><span>แก้ไขรายการ</span></a>');
		if ($isDeleteable) {
			$ui->add('<a class="sg-action'.($isDeleteable?'':' -disabled').'" href="'.url('garage/code/wage/delete/'.$rs->repairid).'" data-confirm="ต้องการลบรายการนี้ กรุณายืนยัน?" data-rel="none" data-removeparent="tr"><i class="icon -delete"></i><span>ลบรายการ</span></a>');
		} else {
...
function __garage_code_wage_save($shopInfo,$data) {
	...
	$stmt='INSERT INTO %garage_repaircode%
		...
		VALUES
		(:repairid, :shopid, 3, :repaircode, :repairname,
...
function __garage_code_wage_delete($shopInfo,$id) {
	if (!__garage_code_wage_isdeleteable($id)) return false;
...
function __garage_code_wage_isdeleteable($id) {
