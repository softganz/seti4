...
function garage_recieve_view($self, $rcvInfo = NULL, $action = NULL) {
	...
	$formOptions = '
	{
		...
		page2 : {
			title: "สำเนาใบเสร็จรับเงิน<span>(เอกสารออกเป็นชุด)",
			for: "(แผนกบัญชี 1)"
		},
		page3 : {
			title: "ใบเสร็จรับเงิน<span>(เอกสารออกเป็นชุด)</span>",
		...
		page4 : {
			title: "สำเนาใบเสร็จรับเงิน<span>(เอกสารออกเป็นชุด)</span>",
			for: "(แผนกบัญชี 3)"
		}
	}';
	...
	switch ($action) {
		...
		default:
			$ret .= '<div class="garage-recieve -main box -no-print">';
	...
	.garage-recieve .row.-header {}
		...
		.garage-recieve address {padding:0;font-style:normal; font-size:0.9em; white-space: nowrap;}
	...
	.garage-recieve .-billid .label {display: inline-block; width:8em; white-space: nowrap;}
	...
	@media print {
		...
		.garage-recieve .-billid .label {width:6.5em;}
...
function __garage_recieve_view_rcvtr($shopInfo,$rcvInfo) {
	...
	$tables->thead = array(
		'center -no -nowrap' => 'ลำดับ<br />Item',
	...
	foreach ($rcvInfo->qt as $rs) {
		...
		if ($rcvInfo->showsingle) {
			...
			$tables->rows[] = array(
				$rcvInfo->showno ? ++$no : '',
				'<div class="detail">'
				// . 'ค่าจัดทำเว็บไซต์และแอปพิเคชั่น'
				. 'ปรับปรุงโปรแกรมระบบบริหารกองทุนสุขภาพตำบลภาคใต้แบบออนไลน์ (www.localfund.happynetwork.org) เขต 12 สงขลา'
				. '</div>',
