 ...
 */
function garage_api_damage($q=NULL,$n=NULL,$p=NULL) {
	...
	$stmt='SELECT d.`damagecode`, d.`damagename`, d.`pretext`
					FROM %garage_damage% d
					WHERE d.`shopid` IN (:shopbranch) AND (d.`damagecode` LIKE :q OR d.`damagename` LIKE :q)
					ORDER BY d.`sorder`, CONVERT(d.`damagename` USING tis620) ASC
					LIMIT '.($p-1).','.$n;
	...
	foreach ($dbs->items as $rs) {
		...
		$desc.=$rs->damagename;
		$result[] = array(
									'value'=>htmlspecialchars($rs->damagecode),
									'label'=>htmlspecialchars($rs->damagecode),
									'pretext'=>htmlspecialchars($rs->pretext),
