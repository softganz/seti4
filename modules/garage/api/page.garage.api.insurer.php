 ...
 */
function garage_api_insurer($q=NULL,$n=NULL,$p=NULL) {
	...
	$stmt='SELECT i.`insurerid`, i.`insurername`, i.`insurerphone`
		FROM %garage_insurer% i
		WHERE i.`shopid` IN (:shopbranch) AND (i.`insurername` LIKE :q OR i.`insurerphone` LIKE :q)
