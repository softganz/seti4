 ...
 */
function garage_api_ap($q=NULL,$n=NULL,$p=NULL) {
	...
	$stmt='SELECT c.`apid`, c.`apname`, c.`apphone`
					FROM %garage_ap% c
					WHERE c.`shopid` IN (:shopbranch) AND (c.`apname` LIKE :q OR c.`apphone` LIKE :q)
					ORDER BY CONVERT(c.`apname` USING tis620) ASC
					LIMIT '.($p-1).','.$n;
	...
	foreach ($dbs->items as $rs) {
		$desc='';
		$desc.=$rs->apphone?'โทร '.$rs->apphone:'';
		$result[] = array(
									'value'=>htmlspecialchars($rs->apid),
									'label'=>htmlspecialchars($rs->apname),
									'phone'=>htmlspecialchars($rs->apphone),
