<?php
function r_garage_stock_cost_calculate_v2($stkid=NULL,$shopInfo=NULL) {
	$useCompoundStock=true;

	// FIXME: Get Shop network list from table garage_shop
	$shopNetwork=array();
	if ($useCompoundStock) {
		$shopNetwork[1]=array(1,2);
		$shopNetwork[2]=array(1,2);
	}

	if (empty($stkid) && empty($shopInfo)) {
		return;
	} else if (empty($stkid)) {
		...
		// Set balance for stkid card
		mydb::query('UPDATE %garage_stocktran% SET `balanceamt`=`qty`, `balancecost`=`total` WHERE `qty`>0 AND `stkid`=:stkid',':stkid',$stkid);
		mydb::query('UPDATE %garage_stocktran% SET `price`=0,`total`=0 WHERE `qty`<=0 AND `stkid`=:stkid',':stkid',$stkid);
	}
	...
	// Select stock id request for calculate
	if ($stkid) {
		$stmt='SELECT `shopid` FROM %garage_stocktran% WHERE `stkid`=:stkid LIMIT 1;';
		$shopid=mydb::select($stmt, ':stkid',$stkid)->shopid;
		$shopInfo=R::Model('garage.get.shop',$shopid);
		$ret.='SHOP'.print_o($shopInfo);
		$shopList=array();

		if ($useCompoundStock) {
			$shopList=$shopNetwork[$shopid];
			$stockLocId=$shopInfo->shopparent;
		} else {
			$shopList[]=$shopid;
			$stockLocId=$shopid;
		}

		$stmt='SELECT DISTINCT :stockLocId `shopid`,`stkid`
					FROM %garage_stocktran%
					WHERE `shopid` IN (:shopid) AND `stkid`=:stkid
					ORDER BY `stkid` ASC;';
		$dbs=mydb::select($stmt, ':shopid','SET:'.implode(',', $shopList), ':stkid',$stkid,':stockLocId',$stockLocId);
	} else {
		$stmt='SELECT DISTINCT `shopid`, `stkid`
					FROM %garage_stocktran%
	...
	foreach ($dbs->items as $stockID) {
		$rowId=$stockID->shopid.$stockID->stkid;
		$tables->rows[$rowId.'-h1']=array('<td colspan="15" style="text-align:left;"><h2>Calculate Shop = '.$stockID->shopid.' Stock ID = '.$stockID->stkid.'</h2></td>');
		$tables->rows[$rowId.'-h2']='<header>';

		// Get Stock Transaction
		$shopList=array();
		if ($useCompoundStock) {
			$shopList=$shopNetwork[$stockID->shopid];
		} else {
			$shopList[]=$stockID->shopid;
		}
		$stmt='SELECT tr.*, FROM_UNIXTIME(tr.`created`,"%Y-%m-%d %H:%i%s") `created`
					...
					ORDER BY
						...
						`stktrid` ASC;
					-- {key:"stktrid"}';
		$stkidDbs=mydb::select($stmt, ':shopid','SET:'.implode(',', $shopList), ':stkid',$stockID->stkid);
		$ret.='<p>Get Stock Transaction :: '.mydb()->_query.'</p>';
		$ret.=print_o($stkidDbs,'$stkidDbs');
