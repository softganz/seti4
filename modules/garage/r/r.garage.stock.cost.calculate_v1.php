<?php
function r_garage_stock_cost_calculate_v1($stkid) {
	...
	foreach ($dbs->items as $stockID) {
		$tables->rows[$stockID->shopid.$stockID->stkid.'-h1']=array('<td colspan="15" style="text-align:left;"><h2>Calculate Shop = '.$stockID->shopid.' Stock ID = '.$stockID->stkid.'</h2></td>');
		$tables->rows[$stockID->shopid.$stockID->stkid.'-h2']='<header>';

		// Get Stock Transaction
		$stmt='SELECT tr.*, FROM_UNIXTIME(tr.`created`,"%Y-%m-%d %H:%i%s") `created`
					FROM %garage_stocktran% tr
					WHERE `shopid`=:shopid AND `stkid`=:stkid
					ORDER BY
						`stkdate` ASC,
						CASE
							WHEN `qty`>0 THEN 0
						...
						`stktrid` ASC;
					-- {key:"stktrid"}';
		$stkidDbs=mydb::select($stmt, ':shopid',$stockID->shopid, ':stkid',$stockID->stkid);
		//$ret.=mydb()->_query;
