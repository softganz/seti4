...
* @return String
*/
function r_set_balance_repair($symbol) {
	if (!$symbol) {
	...
	$ret .= '<h2>Repair balance @'.date('Y-m-d H:i:s').'</h2>';
	$ret .= '<nav class="nav -sg-text-right"><a class="btn -primary sg-action" href="'.url('set/repairbalance/'.$symbol).'" data-rel="#set-info"><i class="icon -material">done</i><span>REPAIR AGAIN</span></a></nav>';
