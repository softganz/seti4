...
class MyApi extends PageApi {
	...
	public function passwordChange() {
		...
		else if (!($post->current === sg_decrypt($userInfo->password,cfg('encrypt_key')))) return error(_HTTP_ERROR_NOT_ACCEPTABLE, 'รหัสผ่านปัจจุบันไม่ถูกต้อง'.$post->current);
		...
		return success('New password was change : บันทึกรหัสผ่านใหม่เรียบร้อย'.mydb()->_query);
	}
}
?>