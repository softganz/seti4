...
class ImageApi extends PageApi {
	...
	function api_image() {
		...
		if ( $debug) {
			debugMsg($para, '$para');
			debugMsg('source file location <em>' . $src_file . '</em>');
		}
		...
		if ( file_exists($src_file) and is_file($src_file) ) {
			...
			if ( $debug ) {
				debugMsg('image file <em>'.$src_file.'</em> not exists.');
			} else {
		...
		if ( $debug ) debugMsg($srcTypes,'$srcTypes');
		...
		if ( $debug ) debugMsg('destination type is <em>'.$dstType.'</em>');

		$srcImg = '<img src="AAA'.url(q(),'src='.$para->src).'">';
		if ( $debug ) debugMsg('image url is <em>'.htmlspecialchars($srcImg).'</em>');
		...
		if (($srcType == "image/jpeg" or $srcType == "image/pjpeg") and function_exists("imagecreatefromjpeg"))
			...
			if ( $debug ) {
				debugMsg('image type <em>'.$srcType.'</em> not support.<br />'.$srcImg);
			} else self::imageDumpFile($src_file);
			return false;
		}
		if (!$handle) {
			if ( $debug ) {
				debugMsg('cannot create image handle.<br />'.$srcImg);
			} else self::imageDumpFile($src_file);
			return false;
		}
		...
		if ( !function_exists("imagecopyresampled") or !function_exists("imagejpeg") ) {
			if ( $debug ) {
				debugMsg("function <b>imagecopyresampled</b> or <b>imagejpeg</b> not support.<br>$srcImg");
			} else self::imageDumpFile($src_file);
			return false;
		}
		...
		if (!$newHandle) {
			if ( $debug ) {
				debugMsg("function imagecreatetruecolor not support.<br>$srcImg");
			} else self::imageDumpFile($src_file);
			return false;
		}
		...
		if (!@imagecopyresampled($newHandle, $handle, 0, 0, $xOffset, $yOffset, $dstWidth, $dstHeight, $cpyWidth, $cpyHeight)) {
			if ( $debug ) {
				debugMsg("imagecopyresampled not support.<br>$srcImg");
			} else self::imageDumpFile($src_file);
			return false;
		}
		@imagedestroy($handle);
		...
		if ( $debug ) {
			debugMsg('<p><img src="'.url(q(),'src='.$para->src).'"></p>');
			debugMsg('<p><img src="'._URL.$para->src.'"><br />source file is '.$src_file.'</p>');
			return true;
		}
	...
	function imageDumpFile($src_file=NULL) {
		if ( file_exists($src_file) and is_file($src_file) ) {
			$srcTypes = getimagesize($src_file);
