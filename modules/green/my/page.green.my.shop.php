...
function green_my_shop($self, $shopId = NULL, $action = NULL) {
	//if (is_numeric($action)) {$shopId = $action; unset($action);}

	if (!i()->ok) {
		// Show Login Page
		new Toolbar($self,'@Secure Log in','none');
		$ret = R::View('signform', '{time:-1, rel: "box", signret: "green/my/shop"}');
	...
	if (!$myShopList) {
		// Shop Registers
		$ret .= '<section class="-sg-text-center"><p class="notify">ยังไม่ได้เปิดร้านค้าบน Green Smile<br /></p><p style="padding: 64px 0;"><a class="sg-action btn -primary" href="'.url('green/my/shop/create').'" data-rel="box" data-width="640">สร้างกลุ่ม/ร้านค้าใหม่บน Green Smile</a></p>';
	...
	new Toolbar($self,'My Shop @Green Smile','my.shop');
	...
	if (!$shopId || post('selectshop')) {
		...
		foreach ($myShopList as $rs) {
			...
			$cardStr = '<div class="-banner"><a href="'.$shopUrl.'"><img class="-logo" src="'.$shopBanner.'" width="96" height="96" /></a></div>'
				...
				. '<nav class="nav -card -sg-text-center"><a class="btn -primary" href="'.url('green/my/shop/'.$rs->shopid).'"><i class="icon -material">done</i><span>เลือกร้านค้า</span></a></nav>';
	...
	if (!$isAdmin && !$myShopList[$shopId]) {
		...
		return message('error', 'SORRY!!!. Group not exists or not your group.');
	}
	...
	switch ($action) {
		...
		default :
			...
			//debugMsg('PAGE IBUY/SHOP ShopId = '.$shopId.' , Action = green.my.'.$action.' , ArgIndex = '.$argIndex.' , Arg = '.func_get_arg($argIndex));
