...
function green_plant_view($self, $plantId) {
	$ret = '';

	$plantId = ($plantInfo = R::Model('green.plant.get', $plantId)) ? $plantInfo->plantId : NULL;
	$shopInfo = R::Model('green.shop.get', $plantInfo->info->orgid);

	$isShopAdmin = $shopInfo->is->orgadmin;

	new Toolbar($self,'จองผลิตภัณฑ์ @Green Smile');
	...
	$form = new Form(NULL, url('green/my/info/book.save/'.$plantId), NULL, 'sg-form -book-form');
	...
	$ret .= '<div class="ibuy-product-shop"><h5>'
		. '<a class="sg-action" href="'.url('green/shop/'.$plantInfo->orgId).'" data-webview="'.$plantInfo->info->shopName.'">'.$plantInfo->info->shopName.'</a> / '
		. '<a class="sg-action" href="'.url('green/land/'.$plantInfo->landId).'" data-webview="'.$plantInfo->info->landName.'">'.$plantInfo->info->landName.'</a>'
		. '</h5>'
	...
	foreach ($dbs->items as $rs) {
		if ($rs->uid == i()->uid || $isShopAdmin) {
			$myBookUi->add('<span class="profile-photo"><img src="'.model::user_photo($rs->bookByUsername).'" width="100%" height="100%" /></span>'
				...
				. ($isShopAdmin ? '<a class="sg-action btn -link" href="'.url('green/my/info/book.remove/'.$rs->bookid).'" data-rel="notify" data-done="load" data-title="ลบรายการจอง" data-confirm="ลบรายการจองนี้ กรุณายืนยัน?"><i class="icon -material -gray">cancel</i></a>' : '')
			);
		}
	}
