...
$debug = true;
function green_activity_form($self) {
	...
	if (!i()->ok) {
		$ret .= R::View(
			...
			'{
				time:-1,
				rel: "none",
				signret: "green/activity/form",
				done: "'.(R()->appAgent ? 'load | ' : '').'load->clear:box:'.url(q()).'",
				regRel: "'.(R()->appAgent ? '#main' : 'box').'"
			}'
		);
	...
	$form = new Form([
		'action' => url('green/my/info/activity.save'),
		'class' => 'sg-form -upload green-activity-form',
		'rel' => 'notify',
		'checkValid' => true,
		'enctype' => 'multipart/form-data',
		'done' => 'close | load:#green-activity-card',
	]);
	...
	//$navUi->add('<div class="" style="margin-right: 4px;"><span class="btn btn-success fileinput-button"><i class="icon -material">photo_camera</i><span>ภาพถ่าย</span><input type="file" name="photo" id="green-activity-photo"  accept="image/*;capture=camera" capture="camera"  /></span></div>');
	...
	function saveNewLocation($this) {
		var saveUrl = "'.url('green/my/info/mobile.location.save').'"
		var para = {}
