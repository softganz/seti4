...
function green_plant($self, $plantId = NULL) {
	// No goodsId, Show Goods Information
	if ($plantId) return R::Page('green.plant.view', $self, $plantId);


	new Toolbar($self,'จองผลผลิต @Green Smile','shop');
	...
	$stmt = 'SELECT SQL_CALC_FOUND_ROWS
		...
		, l.`landname`
		, o.`name`
		, (SELECT `file` FROM %topic_files% WHERE `orgid` = fp.`orgid` AND `tagname` = "GREEN,PLANT" AND `refid` = fp.`plantid` ORDER BY `fid` ASC LIMIT 1) `plantPhotos`
