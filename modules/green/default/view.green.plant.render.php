...
function view_green_plant_render($treeInfo, $options = '{}') {
	$ret = '';
	...
	*/
	...
	if ($isItemEdit) {
		$dropUi->add('<a class="sg-action" href="'.url('green/my/plant/form/'.$treeInfo->plantid).'" data-rel="box" data-width="640"><i class="icon -material">edit</i><span>แก้ไขรายละเอียด</span></a>');
		$dropUi->add('<a class="sg-action" href="'.url('green/my/info/activity.delete/'.$treeInfo->msgid).'" data-rel="notify" data-done="remove:parent .ui-card>.ui-item" data-title="ลบ" data-confirm="ต้องการลบรายการนี้ กรุณายืนยัน?"><i class="icon -material">delete</i><span>ลบรายการ</span></a>');
	}
