...
function green_activity($self, $start = 0) {
	$showItems = 10;
	$uid = i()->uid;
	...
	*/
	...
	foreach ($dbs->items as $rs) {
		$ui->add(
			...
			'{class: "-green-activity -'.strtolower(str_replace(',', '-', $rs->tagname)).'", id: "green-activity-'.$rs->msgid.'"}'
		);
	}
	...
	if ($dbs->_num_rows && $dbs->_num_rows == $showItems) {
		$ret .= '<div id="more" class="green-activity-more" style="padding: 24px 16px 44px;">'
			. '<a class="sg-action btn -primary" href="'.url('green/activity/'.($start+$dbs->_num_rows)).'" data-rel="replace:#more" style="margin:0 auto;display:block;text-align:center; padding: 16px 0;">'
			. '<span>{tr:More}</span>'
