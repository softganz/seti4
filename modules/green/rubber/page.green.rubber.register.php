...
function green_rubber_register($self) {
	$getRefUrl = post('ref');
	...
	if (post('name')) {
		...
		if ($result->shopId) {
			...
			location('green/rubber/my/land');
		}
	}

	// Shop Registers
	$ret .= '<section id="ibut-shop-verify" class="card -margin">'
		. '<p class="-sg-text-center" style="padding: 32px 0;"><big>ยินดีต้อนรับ <b>'.i()->name.'</b> เข้าสู่ระบบการจัดการข้อมูล <b>"สวนยางยั่งยืน"</b></big><br /><br /><br />'
		. '<span>ท่านยังไม่เคยใช้งานมาก่อน ต้องการเริ่มใช้งานหรือไม่?<br /><br /><br /><br />'
		. '<a class="sg-action btn -primary" href="#green-rubber-register-template" data-rel="parent:span" data-width="640">สมัครสมาชิก "สวนยางยั่งยืน"</a></span>'
		. '</p>';
	$ret .= '<p style="padding: 32px;">'
		. '* การสมัครสมาชิก "สวนยางยั่งยืน" แสดงว่าท่านยอมรับข้อตกลงในการใช้งานระบบการจัดการข้อมูล "สวนยางยั่งยืน" เรียบร้อยแล้ว<br />'
	...
	$form = new Form(NULL, url('green/rubber/register'), 'green-rubber-register', 'sg-form');
...
//			'posttext' => '<div class="input-append"><span><a class="-sg-16" href="javascript:void(0)" onclick=\'$("#edit-searchqt").val("");$(this).closest("form").submit()\'><i class="icon -material -gray -sg-16">clear</i></a></span></div>',
	...
	$ret .= '<div id="green-rubber-register-template" class="-hidden"><div class="-sg-text-left">'.$form->build().'</div></div>';
...
function green_rubber_register_old($self) {
	$getRefUrl = post('ref');
	...
	if (post('name')) {
		...
		$result = R::Model('green.shop.create', $data);
		//$ret .= print_o($result,'$result');
		if ($result->shopId) location('green/rubber/my');
	}
	...
	/*
	$ret .= '<section id="ibuy-shop-verify" class="card -margin">'
		...
		. '<span>ท่านยังไม่เคยใช้งานมาก่อน ต้องการเริ่มใช้งานหรือไม่?<br /><br /><br /><br />'
		. '<a class="sg-action btn -primary" href="#green-rubber-register-template" data-rel="parent:span" data-width="640">สมัครสมาชิกเกษตรอินทรีย์</a></span>'
		. '</p>';
	$ret .= '<p style="padding: 32px;">'
	...
	*/
	...
	$ret .= '<section id="ibuy-shop-verify" class="card -margin">'
		. '<p class="-sg-text-center" style="padding: 32px 0;"><big>ยินดีต้อนรับ <b>'.i()->name.'</b> เข้าสู่ระบบการจัดการข้อมูล  <b>"สวนยางยั่งยืน"</b></big><br /><br /><br />'
		. '<span>ท่านยังไม่เคยใช้งานมาก่อน ต้องการเริ่มใช้งานหรือไม่?<br /><br /><br /><br />'
		. '<a class="sg-action btn -primary" href="#green-rubber-register" data-rel="parent:span" data-width="640">สมัครสมาชิกสวนยางยั่งยืน</a></span>'
		. '</p>';
	$ret .= '<p style="padding: 32px;">'
		. '* การสมัครสมาชิกสวนยางยั่งยืน แสดงว่าท่านยอมรับข้อตกลงในการใช้งานระบบการจัดการข้อมูล "สวนยางยั่งยืน" เรียบร้อยแล้ว<br />'
		. '** กรณีที่ท่านเป็นสมาชิกของกลุ่ม/เครือข่ายที่ได้สร้างกลุ่ม/องค์กร/หน่วยงาน/ร้านค้าไว้ในระบบแล้ว ท่านสามารถแจ้งผู้ดูแลกลุ่ม/เครือข่ายให้เพิ่มชื่อของท่านเข้าเป็นสมาชิกของกลุ่ม/เครือข่ายเพื่อจัดการข้อมูลร่วมกัน โดยไม่จำเป็นต้องสร้างกลุ่ม/ร้านค้าใหม่'
		. '</p>';
	$ret .= '</section>';
	...
	$form->addField(
		...
		array(
			...
			'placeholder' => 'ระบุชื่อเครือข่าย หรือ องค์กร'
		)
	);
...
//			'posttext' => '<div class="input-append"><span><a class="-sg-16" href="javascript:void(0)" onclick=\'$("#edit-searchqt").val("");$(this).closest("form").submit()\'><i class="icon -material -gray -sg-16">clear</i></a></span></div>',
	...
	$ret .= '<div id="ibuy-shop-create" class="-hidden">'.$form->build().'</div>';
	return $ret;
}
?>