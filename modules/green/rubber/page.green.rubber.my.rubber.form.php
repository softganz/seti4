...
function green_rubber_my_rubber_form($self, $plantId = NULL) {
	$getLandId = post('land');
	$landList = Array();
	$data = new stdClass();
	$orgInfo = NULL;
	...
	$form->addField('tagname',array('type' => 'hidden', 'value' => 'GREEN,RUBBER'));

	$form->addField('message',array('type' => 'hidden', 'value' => 'ปลูกยาง'));
	...
	*/
	...
	$form->addField('catid', array('type'=>'hidden','value'=>8));
	...
	$form->addField(
		...
		array(
			...
			'class' => '-fill',
			'require' => true,
			'value' => SG\getFirst($data->productname,'ยาง'),
			'placeholder' => 'ระบุชื่อพันธุ์ยาง',
		)
	);
