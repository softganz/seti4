...
function green_my_rubber($self) {
	...
	new Toolbar($self, $shopInfo->name.' @Green Smile','my.rubber');

	$headerNav = new Ui();
	$headerNav->addConfig('nav', '{class: "nav -page -sg-text-right"}');

	$headerNav->add('<a class="btn -link" href="'.url('green/my/goods', array('show' => 'all')).'">ALL</a>');
	$headerNav->add('<a class="btn -link" href="'.url('green/my/goods', array('show' => 'out')).'">OUT</a>');
	$headerNav->add('<a class="btn -link" href="'.url('green/my/goods', array('show' => 'no')).'">NO</a>');
	$headerNav->add('<a class="sg-action btn -primary" href="'.url('green/my/goods/form').'" data-rel="box" data-width="640"><i class="icon -material">add_circle_outline</i><span>เพิ่มสินค้า</span></a>');
