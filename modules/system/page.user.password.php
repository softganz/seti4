...
function user_password($self) {
	$self->theme->title = 'Password request';

	$self->theme->header->description = R::View('user.menu','password');

	if (!mydb::columns('%users%','pwresettime')) {
		mydb::query('ALTER TABLE %users% ADD `pwresettime` BIGINT NULL DEFAULT NULL AFTER `code`, ADD INDEX (`pwresettime`);');
	}
	...
	if ($post->username) {
		...
		if ( $mail->IsValidEmail($username) ) { // input email
			mydb::where('`email`=:email',':email',$username);
			$sql_cmd = 'SELECT * FROM %users% WHERE email="'.$username.'"';
			$get_by_email=$username;
		} else { // input username
			mydb::where('`username`=:username',':username',$username);
			$get_by_email='ตามที่ท่านได้ลงทะเบียนไว้ในชื่อ '.$username;
		...
		if ($result->_num_rows) {
			...
			foreach ($result->items as $rs) {
				...
				// send mail
				$title = 'Password request from '.cfg('domain');
				$from = 'noreply@'.cfg('domain.short');
				$from_name = 'noreply';
				$message = 'Password request from <strong>'.cfg('domain').'</strong><br /><br />'.$password_detail.'<hr />Please do not reply this mail';
				$mail->FromName($from_name);
				...
				if ( $mailto ) {
					$mail_result = $mail->Send($mailto,$title,$message,false,'https://service.softganz.com');
					if ($mail_result) {
						$ret.=message('status','ได้ส่งข้อมูลไปให้ท่านที่อีเมล์ <strong>'.$get_by_email.'</strong> เรียบร้อยแล้ว : <ul><li>กรุณาตรวจสอบอีเมล์ตามที่ท่านระบุไว้ในทะเบียนสมาชิก และคลิกลิงก์เพื่อเปลี่ยนรหัสผ่าน</li><li>หากยังไม่ได้รับอีเมล์ ให้ลองตรวจสอบใน <b>เมล์ขยะ (Spam mail)</b> ด้วยเพราะอาจจะถูกนำไปเก็บไว้ในนั้น</li></ul>');
						$logMsg.=' And password is send to email complete.';
						$stmt='UPDATE %users% SET `code`=:token, `pwresettime`=:now WHERE `uid`=:uid LIMIT 1';
						mydb::query($stmt,':token',$token, ':now',date('U'),':uid',$rs->uid);
					} else {
		...
		} else $ret .= message('error','ไม่มีข้อมูลสมาชิกตามรายละเอียดที่ท่านระบุ');
	...
	$form = new Form('request',url(q()),'password-request','sg-form');
	...
	$form->addField('send',array('type'=>'button','value'=>'Send password'));

	$form->addField('help',array('type'=>'textfield','value'=>'ให้ท่านป้อน username หรือ email address ที่ท่านลงทะเบียนไว้กับเว็บไซท์ แล้วระบบจะส่งรหัสผ่านไปให้ท่านยัง email ที่ท่านได้ทำการลงทะเบียนไว้'));
