...
class AdminAppBarWidget extends AppBar {
	...
	private function navigator() {
		return array_merge(
			[
				'<a href="'.url('admin').'"><i class="icon -material">home</i><span class="">Home</span></a>',
				//'<a href="'.url('admin/task').'">'.tr('By task').'</a>',
				//'<a href="'.url('admin/module').'">'.tr('By modules').'</a>',
				'<a href="'.url('admin/content').'"><i class="icon -material">ballot</i><span>{tr:Content}</span></a>',
				'<a href="'.url('admin/site').'"><i class="icon -material">build</i><span>Site</span></a>',
				'<a href="'.url('admin/user').'"><i class="icon -material">people</i><span>Users</span></a>',
				'<a href="'.url('admin/config').'"><i class="icon -material">web</i><span>Config</span></a>',
				'<a href="'.url('admin/log').'"><i class="icon -material">check_box</i><span>Logs</span></a>'
			],
			$activeUrl === 'site' ? [
