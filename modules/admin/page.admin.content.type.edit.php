...
function admin_content_type_edit($self,$type_id) {
	...
	$type_db=BasicModel::get_topic_type($type_id);
	...
	if ($type->name) {
		...
		if ($type->type && $type->type!=$type_db->type && BasicModel::get_topic_type($type->type)) $error[]='Type name <b>'.$type->type.'</b> is inused.';
		if ($error) {
