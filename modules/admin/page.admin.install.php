...
function admin_install($self) {
	...
	if (!mydb::table_exists('%variable%')) {
		...
		if ($result->error) {
			$ret.= message('error','First time installation database error');
			$ret.='<h3>Install table error.</h3><ul><li>'.implode('</li><li>',$result->error).'</li></ul>';
		} else {
			__admin_install_set_default_config();
			location('user/register');
		}
		if ($result->table) $ret.='<h3>Install table completed.</h3><ul><li>'.implode('</li><li>',$result->table).'</li></ul>';
		if ($result->module) $ret.='<h3>Install module.</h3>'.implode('',$result->module);
		return $ret;
	}
 ...
 * @return null
 */
function __admin_install_set_default_config() {
	// add default module
...
function __admin_install_create_table($prefix=null) {
	...
	foreach ($query as $table=>$sql_cmd) {
		if (!mydb::table_exists($table)) {
			...
			if (mydb()->_error) {
				$result->error[] = mydb()->_query.' : '.mydb()->_error;
			} else {
				$result->complete[]=$prefix.$table;
			}
		}
	}
