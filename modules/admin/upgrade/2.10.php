...
if (!db_is_key_exists('%topic_comments%','timestamp')) {
	...
	$result[$version][]=array(
												'Add key timestamp of topic comment '.db_affectedrows().' row(s) effect.',
												mydb()->_query,
												mydb()->_error,
												mydb()->_error_no
											);
...
$result[$version][]=array(
											'Add wating flag to users status.',
											mydb()->_query,
											mydb()->_error,
											mydb()->_error_no
										);
...
mydb::query('ALTER TABLE  %calendar% ADD  `tpid` INT UNSIGNED NULL DEFAULT NULL AFTER  `id` , ADD INDEX (  `tpid` )');
$result[$version][]=array(
											'Add field tpid to calendar',
											mydb()->_query,
											mydb()->_error,
