...
function forum_home($self, $forumId = NULL) {
	...
	$vocab = DB::select([
		'SELECT * FROM %vocabulary_types% WHERE `type` = "forum" LIMIT 1'
	]);

	if ($vocab->_nuvidm_rows) {
		$tree = BasicModel::get_taxonomy_tree($vocab->vid);
		$topic_count = DB::select([
			'SELECT tid,COUNT(*) AS topics FROM %tag_topic% WHERE vid = :vid GROUP BY tid',
			'var' => [':vid' => $vocab->vid]
		]);
	...
	$result = new Widget([
		'children' => [
			user_access('create forum content') ? '<nav class="nav -page -sg-text-right"><a class="btn -primary" href="'.url('paper/post/forum').'"><i class="icon -material">add_circle</i><span>{tr:Post new Forum topic}</span></a></nav>' : null,
			$tree ? new Table([
				'id' => 'forum-list',
				'thead' => [
					'name' => 'Forum',
