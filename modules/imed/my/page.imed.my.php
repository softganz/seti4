...
function imed_my($self) {
	...
	$mainUi->add('<a class="sg-action" href="'.url('signout').'" data-rel="none" data-title="ออกจากระบบสมาชิก" data-confirm="ต้องการออกจากระบบสมาชิก กรุณายืนยัน?" data-done="reload:'.url('imed/app').' | moveto:0,0"><i class="icon -material">lock_open</i><span>{tr:Sign out}</span></a>');
	...
	$mainUi = new Ui(NULL, NULL);
	$mainUi->addConfig('nav', '{class: "nav -app-menu"}');
	$mainUi->header('<h3>รายงานตามพื้นที่</h3>');
	...
	if (i()->username == 'softganz') {
		$otherUi->add('<a class="sg-action" href="'.url('imed/care').'" data-webview="iMedCare" data-options=\'{history: true, menu: [{id: "account", label: "Giver", title: "ผู้ให้บริการ", link="'.url('imed/care/giver').'"},{id: "group", label: "Team", title: "iMedCare Team", link="'.url('imed/care/team').'"}]}\'><i class="icon"><img src="//communeinfo.com/themes/default/care/logo_care.png" width="24" height="24" /></i><span>iMedCare</span></a>');
		$otherUi->add('<a class="sg-action" href="'.url('imed/psyc').'" data-webview="iMed@Psyc"><i class="icon -material">psychology</i><span>iMed@Psyc</span></a>');
	}
	...
	if ($isAdmin) {
		...
		if (i()->username == 'softganz') {
			if (_DOMAIN_SHORT == 'localhost') {
				if (R()->appAgent) {
					$adminUi->add('<a href="'.url('imed/app/my',array('setting:app' => '')).'"><i class="icon -material">web</i><span>www</span></a>');
				} else {
					$adminUi->add('<a href="'.url('imed/app/my',array('setting:app' => '{OS:%22Android%22,ver:%220.20.00%22,type:%22App%22,dev:%22Softganz%22,theme:%22dark%22}')).'"><i class="icon -material">android</i><span>App</span></a>');
				}
			}
