...
function view_imed_menu_group($orgInfo) {
	$orgId = $orgInfo->orgid;
	...
	$menuUi->add('<a class="sg-action" href="'.url('imed/social/'.$orgId.'/patient').'" data-rel="#imed-app"><i class="icon -material">accessible</i><span>{tr:Patients}</span></a>');
	...
	if ($isAdmin || $isPoCenter) {
		$menuUi->add('<a href="'.url('imed/pocenter/'.$orgId).'" ><i class="icon -material">accessible_forward</i><span>กายอุปกรณ์</span></a>');
	}
	if ($isAdmin) {
		$menuUi->add('<a class="sg-action" href="'.url('imed/social/'.$orgId.'/report').'" data-rel="#imed-app"><i class="icon -material">assessment</i><span>{tr:Report}</span></a>');
	...
	foreach ($dbs->items as $rs) {
		...
		$menuUi->add('<a class="" href="'.url('imed/social/'.$rs->orgid).'" data-rel="#imed-app" title="'.$rs->groupname.'"><i class="icon -material -sg-32">group</i><span>'.$rs->groupname.'</span></a>');
	}
