...
class ImedCareInfoHome extends Page {
	var $patientInfo;
	var $isAccess;
	var $isEdit;
			...
			'appBar' => new AppBar([
				...
				'trailing' => new DropBox([
					...
					'children' => [
						...
						$this->isAdmin ? '<a class="sg-action" href="'.url('imed/qt/view/'.$psnId).'" data-rel="box"><i class="icon -material">list</i><span>รายการแบบสอบถาม</span></a>' : NULL,
					], // children
				]), // Ui
	...
	function _patientMenu() {
		$psnId = $this->patientInfo->psnId;
		$menu = [
			'<a class="sg-action btn -primary -fill" href="'.url('imed/app/'.$psnId.'/info.visit').'" data-webview="เยี่ยมบ้าน"><i class="icon -doctor"></i><span class="lang-text -th">เยี่ยมบ้าน</span><span class="lang-text -en">Home Visit</span></a><span class="-number">'.$this->patientInfo->visit->count.'</span>',
			...
			'<a class="sg-action btn -primary -fill" href="'.url('imed/app/'.$psnId.'/info.po').'" data-webview="กายอุปกรณ์"><i class="icon -material">how_to_reg</i><span class="lang-text -th">กายอุปกรณ์</span><span class="lang-text -en">Orthotics Information</span></a>',
			'<a class="sg-action btn -primary -fill" href="'.url('imed/app/'.$psnId.'/info.vitalsign').'" data-webview="สัญญาณชีพ"><i class="icon -heart"></i><span class="lang-text -th">สัญญาณชีพ</span><span class="lang-text -en">Vital Sign</span></a>',
		...
		if ($this->isAccess) {
			$menu[] = '<a class="sg-action btn -primary -fill" href="'.url('imed/app/'.$psnId.'/info.personal').'" data-webview="ข้อมูลทั่วไป"><i class="icon -person"></i><span class="lang-text -th">ข้อมูลทั่วไป</span><span class="lang-text -en">Personal Information</span></a>';
			$menu[] = '<a class="sg-action btn -primary -fill" href="'.url('imed/app/'.$psnId.'/info.health').'" data-webview="ข้อมูลสุขภาพ"><i class="icon -heart"></i><span class="lang-text -th">ข้อมูลสุขภาพ</span><span class="lang-text -en">Health Information</span></a>';
