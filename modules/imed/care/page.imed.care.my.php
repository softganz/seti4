...
class ImedCareMy extends Page {
	function build() {
		// Access Controle Check from Manifest
		// if (!i()->ok) {
		// 	return new SignForm();
		// }
		...
		$mainUi->add('<a class="sg-action" href="'.url('signout').'" data-rel="none" data-title="ออกจากระบบสมาชิก" data-confirm="ต้องการออกจากระบบสมาชิก กรุณายืนยัน?" data-done="reload:'.url('imed/care').' | moveto:0,0"><i class="icon -material">lock_open</i><span>{tr:Sign out}</span></a>');
		...
		if ($isAdmin) {
			...
			if (i()->username == 'softganz') {
				if (_DOMAIN_SHORT == 'localhost') {
					if (R()->appAgent) {
						$adminUi->add('<a href="'.url('care', ['setting:app' => '']).'"><i class="icon -material">web</i><span>www</span></a>');
					} else {
						$adminUi->add('<a href="'.url('care', ['setting:app' => '{OS:%22Android%22,ver:%220.1.07%22,type:%22App%22,dev:%22Softganz%22,theme:%22dark%22}']).'"><i class="icon -material">android</i><span>App</span></a>');
					}
				}
