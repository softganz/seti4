 ...
 * @return String
 */
function imed_patient_elder($self, $psnId = NULL) {
	...
	if (!$isElder) {
		$ret .= '<div class="-sg-text-center" style="padding: 32px 0;">'
			. '<p class="notify"><strong>'.$psnInfo->fullname.'</strong> ไม่ได้อยู่ในกลุ่มของผู้สูงอายุ</p>'
			. ($isEdit ? '<p style="padding: 32px 0;">ต้องการเพิ่ม <strong>'.$psnInfo->fullname.'</strong> เข้าไว้ในกลุ่มผู้สูงอายุหรือไม่?</p><p><a class="sg-action btn -primary" href="'.url('imed/patient/'.$psnId.'/info/elder.add').'" data-rel="#imed-app" data-ret="'.url('imed/patient/elder/'.$psnId).'"><i class="icon -addbig -white"></i><span>เพิ่มเข้ากลุ่มผู้สูงอายุ</span></a>' : '')
			. '</div>';
		return $ret;
	}
	...
	$dropUi = new Ui();
	if ($isEdit) {
		if ($psnInfo->care->elder) {
			$dropUi->add('<a class="sg-action" href="'.url('imed/patient/'.$psnId.'/info/elder.remove').'" data-rel="#imed-app" data-ret="'.url('imed/patient/elder/'.$psnId).'" data-title="ลบชื่อออกจากกลุ่มผู้สูงอายุ" data-confirm="ต้องการลบชื่อออกจากกลุ่มผู้สูงอายุ กรุณายืนยัน?"><i class="icon -material">cancel</i><span>ลบออกจากกลุ่มผู้สูงอายุ</span></a>');
		}
	}
	if ($dropUi->count()) $ui->add(sg_dropbox($dropUi->build()));

	$ret .= '<header class="header"><h3>ข้อมูลผู้สูงอายุ'.($psnInfo->info->dischar == 1 ? ' (เสียชีวิต)' : '').'</h3><nav class="nav -page -sg-text-right">'.$ui->build().'</nav></header>';



	include_once 'modules/imed/assets/qt.elder.php';

	$inlineAttr['class'] = 'imed-qt -elder ';
	...
	$ret .= '<div id="imed-care-elder" '.sg_implode_attr($inlineAttr).'>'._NL;
