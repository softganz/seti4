...
* @return String
*/
function r_flood_forcast_hr3_read($file, $polygon = NULL, $shapeBox = array(), $polygonArray = array()) {
	...
	if ($polygonArray) {
		if ($debug) debugMsg('IN POLYGON '.$file);

		foreach ($polygonArray as $j => $row) {
			$y = $yllcenter + ($rowCount-1-$j) * $cellsize;
			foreach ($row as $i => $v) {
				$x = $xllcenter + $i*$cellsize;
				$rainCellValue = $cells[$j][$i];
				$sumRain += $rainCellValue;
				$areaCount++;
	...
	if ($debug) debugMsg('NOT IN POLYGON '.$file);
