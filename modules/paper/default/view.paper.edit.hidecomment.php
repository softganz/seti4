...
function view_paper_edit_hidecomment($topicInfo, $commentId) {
	...
	CommonModel::watch_log('paper','Paper comment '.$rs->status,'Comment '.$rs->cid.' of <a href="'.url('paper/'.$rs->tpid.'#comment-'.$rs->cid).'">paper/'.$rs->tpid.'</a> was '.sg_status_text($rs->status));
