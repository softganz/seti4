...
class PaperPostFormWidget extends Widget {
	...
	function build() {
		...
		$areaTypeCode = $cfgAreaCode->{$this->topic->type->type}->area;

		return new Form([
			'variable' => 'topic',
			'action' => url(q()),
			'id' => 'edit-topic',
			'class' => 'sg-form paper-post',
			'checkValid' => true,
			'enctype' => 'multipart/form-data',
			'children' => [
				$this->topic->type->help ? sg_text2html($this->topic->type->help) : NULL,
				$this->topic->message ? sg_text2html($this->topic->message) : NULL,
				...
				'orgid' => $this->topic->org ? ['type' => 'hidden', 'value' => $this->topic->org,] : NULL,
				'daykey' => !i()->ok ? [
					'name' => 'daykey',
					'type' => 'text',
				...
				'title' => $this->topic->type->has_title ? [
					'type' => 'text',
					'label' => tr($this->topic->type->title_label),
					'class' => '-fill',
					'maxlength' => 150,
					'require' => true,
					'value' => $this->topic->post->title,
					'placeholder' => 'ชื่อหัวข้อ',
				] : NULL,

				'vid' => self::vid($this->topic),

				'areacode' => $this->topic->type->type && $areaTypeCode ? (function($areaTypeCode) {
				...
				'body' => $this->topic->type->has_body ? [
					'type' => 'textarea',
					'label' => $this->topic->type->body_label,
					'class' => '-fill',
					'rows' => 12,
					'require' => true,
					'value' => $this->topic->post->body,
				...
				'tabs' => new Children([
					'children' => [
						...
						'poster' => [
							...
							'value' => \SG\getFirst($this->topic->post->poster,i()->name),
						],
						'email' => !i()->ok ? [
							'type' => 'text',
							'label' => tr('Email'),
						...
						'document_title' => user_access('upload document') ? [
							...
							'maxlength' => 150,
							'class' => '-fill',
							'value' => $this->topic->post->document_title,
							'placeholder' => 'ชื่อเอกสารประกอบ',
						] : NULL,
						...
						'input_format' => user_access('upload document') ? [
							...
							'value' => $this->topic->post->property['input_format'],
						] : NULL,
				...
				'submit' => [
					...
					'pretext' => '<a class="btn -link -cancel" href="'.url('paper').'"><i class="icon -material -gray">cancel</i><span>'.tr('CANCEL').'</span></a>',
	...
	private static function vid($topic) {
		$fields = (Object) [
			'type' => 'group',
			'children' => [],
		];
